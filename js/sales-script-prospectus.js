  ...
  function riskItems(text) {
    ...
    for (var h = 0; h < heads.length && out.length < 8; h++) {
      ...
      for (var j = start + 1; j < stop; j++) {
        ...
        /*
         * 위험표 맨 왼쪽에 행 번호 칸을 둔 설명서가 있다. 그 칸이 「1」 처럼
         * 숫자만 있는 줄로 떨어져 나오는데, RISK_SECTION 은 「1.」·「1)」 처럼
         * 점이나 괄호가 붙은 것만 걸러 내므로 맨 숫자가 이름 앞에 붙었다.
         * 그래서 창구에서 「1 시장위험 및 개별위험 — …」 을 그대로 읽게 됐다.
         * 판독된 6,005개 위험 항목 중 662개(11.0%)가 이 꼴이었다.
         * 이름을 지우지 말고 그 줄만 건너뛴다 — 번호 다음이 이름이기 때문이다.
         */
  ...
  var RULES = {
    ...
    fund: COMMON_RULES.concat([
      ...
      },
      {
         ...
         *   ㆍAB 「최근 결산일(2025.12.31) 기준 3년 변동성은 7.81%이며」
         * KB 것은 값이 다음 줄에 있어 앞말로는 못 잡는다. 대신 「○○%로 N등급」 이
         * 이 문서에서 실측치에만 쓰이는 꼴이라 그것으로 잡는다.
         * AB 것은 값을 「3년 변동성」 이라고만 부른다 — 앞 문장에서 그 변동성이
         * 「최대손실예상액(97.5% VaR 모형 사용)」 임을 밝히고 있고, 실제로 7.81% 는
         * 같은 쪽 등급기준표의 「10% 이하 = 5등급」 과 이 펀드의 5등급이 맞아떨어진다.
         * 창구가 짚어 준 AB글로벌고수익증권투자신탁이 이 꼴이라 확인필요로 남았다.
        ...
        /(?:\d\s*년|기간)?\s*변동성은\s*(?:약\s*)?([\d.]+)\s*%(?!\s*(?:초과|이하|미만|이상))/,
      ...
      {
        /**
         * 환헤지 여부. 실제 설명서가 쓰는 말은 다양하다 — 창구가 짚어 준
         * AB글로벌고수익증권투자신탁(채권-재간접형)의 원문은 이랬다.
         *
         *   「…미달러화 가치에 대하여 환헤지할 것을 목표로 합니다」
         *   「시장상황에 따라 탄력적으로 환헤지 전략을 실행할 계획입니다」
         *
         * 앞 판은 「환위험을 헤지」 또는 「환헤지 … 수행·실시」 만 봤으므로 둘 다
         * 놓쳤다. 「목표로」 「실행할 계획」 을 함께 본다.
         *
         * ★ 「환헤지란 선물환 계약 등을 이용하여…」 같은 용어 설명과 뒤쪽 용어집에도
         * 「환헤지」 가 나온다. 그것을 여부로 읽으면 헤지하지 않는 펀드까지 실시로
         * 읽히므로, 이 펀드가 무엇을 하는지 서술하는 꼴만 잡는다.
         *
         * 미실시 판정은 좁게 잡는다. 같은 문서에 「환헤지가 이루어지지 않는 부분에
         * 대해서는 환율 변동위험에 노출될 수 있습니다」 가 있는데, 이것은 헤지를 하되
         * 남는 부분을 말하는 문장이지 미실시가 아니다.
         */
        id: 'fxHedge', label: '환헤지 여부·대상통화',
        re: [/환(?:율)?\s*헤지를?\s*(?:실시|수행|실행)하지\s*(?:아니하|않)/,
        /환(?:율)?\s*헤지\s*(?:미실시|을?\s*하지\s*아니하|을?\s*하지\s*않)/,
        /환(?:율)?위험을?\s*헤지/,
        /환\s*헤지\s*할\s*것을\s*목표/,
        /환\s*헤지\s*전략을?\s*(?:실행|수행|실시)/,
        /환\s*헤지를?\s*(?:실행|수행|실시)/],
        map: function (m) {
          if (/않|아니하|미실시/.test(m[0])) return '환헤지 미실시';
          /* 「시장상황에 따라 탄력적으로」 는 조건부다 — 단정해서 읽으면 사실과 어긋난다 */
          return /탄력적|시장\s*상황에\s*따라/.test(m.input.slice(Math.max(0, m.index - 60), m.index))
            ? '시장상황에 따라 환헤지 전략을 실행 (실행 여부와 비율이 시장상황에 따라 달라짐)'
            : '환헤지 실시 (외화표시 투자자산의 환위험 헤지)';
        }
      },
      {
        /**
         * 목표 환헤지 비율. 원문은 「환」 을 빼고 「목표 헤지비율」 로 쓰는 경우가 있고,
         * 값이 한 수치가 아니라 범위인 경우가 있다 —
         *   「일반적인 경우 미달러화의 원화에 대한 목표 헤지비율은 80%에서 +/- 20%입니다」
         * 이때 「약 80%」 라고만 담으면 범위를 빠뜨려 창구가 좁게 말하게 된다.
         */
        id: 'fxHedgeSize', label: '목표 환헤지 비율',
        re: [/목표\s*(?:환)?\s*헤지\s*비율은?[^\n]{0,40}?([\d.]+)\s*%\s*(?:에서|±|\+\/-|부터)\s*(?:\+\/-|±)?\s*([\d.]+)\s*%/,
        /목표\s*(?:환)?\s*헤지\s*비율은?[\s\S]{0,80}?약?\s*([\d.]+)\s*%\s*(?:이상|~)\s*([\d.]+)\s*%\s*이하/,
        /목표\s*(?:환)?\s*헤지\s*비율은?[\s\S]{0,80}?약?\s*([\d.]+)\s*%/],
        map: function (m) {
          if (m[2] == null) return '약 ' + num(m) + '% (해외투자분 순자산 대비)';
          /* 「80%에서 ±20%」 와 「70% 이상 ~ 110% 이하」 를 원문 그대로 옮긴다 */
          return /이상|~/.test(m[0])
            ? m[1] + '% 이상 ~ ' + m[2] + '% 이하 (해외투자분 순자산 대비)'
            : m[1] + '% ± ' + m[2] + '%p (해외투자분 순자산 대비)';
        }
      },
      {
