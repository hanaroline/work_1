...
(function () {
  ...
  function fundCatDoc(it) {
    ...
    if (ex) {
       ...
       * 단정했다. PDF 에서 「없 음」 처럼 글자 사이가 벌어지면 못 읽는데, 그렇게
       * 255건이 「부과」 로 담겼고 그중 222건은 환매수수료 값이 아예 없었다 —
       * 부과된다고 말할 근거가 없다는 뜻이다. 그대로 두면 원문에 「없음」 이라고 쓰인
       * 펀드를 두고 창구가 고객에게 환매수수료가 붙는다고 말하게 된다.
       *
       * 판독기를 고치고 전량을 다시 읽어 지금은 0건이다. 그래도 남겨 둔다 —
       * 근거 없는 단정을 걷어내는 잣대는 데이터가 깨끗해도 값싸고, 다음 수집이
       * 같은 구멍에 빠졌을 때 창구를 지키는 자리다.
      ...
      Object.keys(ex).forEach(function (k) {
         ...
         *
         * 판독분에 표의 클래스 칸이 값 자리에 들어온 것이 섞여 있었다
         * (선취수수료 137건이 「A」·「A-e」·설명 문장, 총보수 77건). 그대로 쓰면
         * 화면은 「등록됨」 인데 계산 문장은 만들어지지 않아, 창구가 무엇이 잘못됐는지
         * 알 수 없다. 값이 아니면 아예 담지 않아 「확인필요」 로 남긴다.
         *
         * 판독기를 고치고 전량을 다시 읽어 지금 담긴 3,003건에는 걸릴 것이 없다
         * (2026-09-10 기준 0건). 그래도 남겨 둔다 — 다음 수집이 규칙의 새 구멍에
         * 걸릴 수 있고, 이 잣대는 그때 창구가 틀린 값을 읽는 것을 막는 마지막 자리다.
         */
