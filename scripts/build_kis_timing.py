...
# 화면이 시장을 늘어놓는 차례. **근거가 두꺼운 것부터** 둔다.
#
#   국내ETF   8해치 · 검증구간 초과수익이 네 시장에서 가장 크고 꾸준하다
#   국내주식   초과수익은 더 크지만 구간이 2해뿐이고 겪은 장세가 하나다
#   해외ETF   우주를 18→31종으로 넓히자 초과수익이 +1.54 → +0.05%p 로 주저앉았다
#   미국주식   검증구간에서 K=1·K=2 가 모두 음수였다 — 이 10종이 서지 않는 자리다
#
# **왜 자료에 싣는가.** 화면과 검사기가 각자 차례를 갖고 있으면 언젠가 갈라진다.
...
def main():
    ...
    out = {
        ...
        # 화면이 쓰는 차례. 없는 시장은 화면이 알아서 건너뛴다.
        'view_order': VIEW_ORDER,
        'view_order_why': ('근거가 두꺼운 시장부터 둡니다 — 검증구간 성적과 자료 구간의 '
                           '길이를 함께 본 차례이고, 성적표를 다시 재도 바뀌지 않습니다.'),
