...
**위험은 셀 수 있는 것만 센다**
──────────────────────────────────────────────────────────────────────
펀드는 달 간격 기준가로 변동성을 낸다(universe 가 그렇게 적어
...
def export_policy(path=POLICY_OUT):
    ...
    doc = {
        ...
        "notes": {
            ...
            "펀드": ("펀드는 달 간격 기준가로 변동성을 셈하므로 일봉으로 "
                     "잰 것과 결이 다릅니다. 계열이 없는 펀드는 원천의 52주 "
                     "표준편차를 쓰고 최대낙폭은 비워 둡니다. 그래서 "
                     "포트폴리오 변동성은 덮은 비중을 함께 적습니다."),
            "상품선정": ("여러 해의 위험조정 성과로 고릅니다. 수익률 순으로도 "
                         "규모 순으로도 고르지 않습니다 — "
