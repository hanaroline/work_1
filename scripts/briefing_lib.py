...
def perf_line(p, unit="%"):
    """핵심본용 기간 사다리 — **열이 아니라 이름 밑의 한 줄로.**

    `THP()`/`perf_cells()` 는 핵심본에서 열 넷을 접는다. 반 칸짜리 격자에 일곱
    열을 세우면 표가 통째로 넘치기 때문이다(재 봤더니 246px). 그런데 접고 나니
    「지금이 신고가인지 되돌림인지」가 핵심본에서만 통째로 없어졌다 &mdash;
    전일 대비 하나만으로는 어제 3% 오른 것이 <strong>회복인지 신고가인지</strong>
    가려지지 않는다.

    그래서 **열 대신 줄로** 되살린다. `fold_perf.py` 가 좁은 화면에서 하는 일과
    같은 모양(`.perfline`)이지만, 그쪽은 화면 폭에 따라 열과 줄을 번갈아 보이므로
    인쇄에서는 숨는다. 핵심본에는 열이 아예 없으니 **항상 보여야 한다** &mdash;
    그래서 클래스를 `perfrow` 로 따로 둔다. 줄 하나는 4px 남짓이라 여섯 쪽을
    깨지 않는다.
    """
    p = p or {}
    f = bp if unit == "bp" else pct
    bits = []
    # 넷을 다 실으면 줄이 반 칸을 넘겨 두 줄이 되고, 표 열아홉 개에서 그 한 줄씩이
    # 쌓여 여섯 쪽이 일곱 쪽이 된다(재 봤더니 +791px). **3개월을 뺀다** — 1주·1개월·
    # 연초면 짧은 쪽·중간·올해가 다 들어와 사다리로서 할 일을 한다. 넉 줄 전부는
    # 같은 날짜의 전체 판에 열로 서 있다.
    for k, lab in (("w1", "1주"), ("m1", "1개월"), ("ytd", "연초")):
        if p.get(k) is None:
            continue
        bits.append('<span class="p"><span class="k">' + lab + '</span> ' + f(p[k]) + '</span>')
    if not bits:
        return ""
    return '<span class="perfrow">' + '<i>&middot;</i>'.join(bits) + '</span>'
