...
def kr_close(j):
    ...
    `index_daily`(네이버 일별시세)가 기준이고 `indices.kospi`(야후 ^KS11)는
    참고다. 야후 쪽은 마감 당일 저녁에는 그날 봉을 주는데 다음 날 새벽에는
    다시 전전 거래일로 되돌아간다 — 8월 11일 06:24 수집이 8월 7일을 줬고,
    같은 파일의 `index_daily` 는 8월 10일이 맞았다. 다시 수집해도 고쳐지지
    않는 원천 성질이라 이것 때문에 낡았다고 판정하지 않는다. 대신 어긋난
    사실을 찍어 준다 — 그날 국내 종가는 `index_daily` 를 쓰라는 뜻이다.
    """
