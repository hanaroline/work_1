...
function parseBlock(text, pre) {
  ...
  /**
   * 만기평가일은 만기일과 다르다 (제38050회: 평가 2029-08-29 / 만기 2029-09-03).
   * 손익구조 문구가 "만기평가일(날짜)에 만기평가가격이…" 라고 읽으므로 평가일을 써야 한다.
   * 만기일을 넣으면 실제와 며칠 어긋난 날짜를 상담에서 읽게 된다.
   */
  p.maturityEvalDate = ymd((text.match(/만기평가일\s*:\s*([^\n○]+)/) || [])[1]);
  ...
  /**
   * 만기상환 조건 — 배리어와 그 조건을 채웠을 때의 투자수익률.
   *
   * 문구가 회차마다 조금씩 다르다. 「모든 기초자산의 …」 만 받으면
   * 「두 기초자산의 만기평가가격이 **모두 각** 최초기준가격의 [500%] 이상인 경우」
   * (제4059·4060·4067·4068회) 를 못 읽어 만기행 자체가 안 만들어졌다.
   * 앞의 수식어는 보지 않고 「기초자산의 만기평가가격이」 부터 읽는다.
   */
  const MAT_COND = '기초자산의\\s*만기평가가격이\\s*(?:모두\\s*)?(?:각\\s*)?최초기준가격의'
    + '\\s*\\[(\\d{1,3}(?:\\.\\d+)?)%\\]\\s*이상인\\s*경우';
  p.maturityBarrier = numOf((text.match(new RegExp(MAT_COND)) || [])[1]);
  /**
   * 만기 지급률의 근거 — 손익구조 표에서 그 조건 바로 뒤에 오는 투자수익률(세전) 칸이다.
   *
   * 앞 판은 이 칸을 안 읽고 문서 전체의 최대 총수익률(docMaxRate)로 대신했다.
   * 그런데 월지급식은 docMaxRate 가 「한 달치」라 그대로 쓰면 3년물 만기 지급률이
   * 100.5% 로 나온다. 그래서 월지급식은 아예 비워 「확인필요」로 남겼는데 —
   * 정작 원문에는 값이 적혀 있다. 월지급식의 만기상환 칸은 예외 없이
   * 「0%(원금지급)」·「0%(액면금액)」·「0%(액면잔액)」 이다. 수익을 매달 따로 주므로
   * 만기에는 원금만 상환한다는 뜻이고, 조기상환 차수가 모두 「액면금액 × 100.00%」
   * 인 것과 같은 말이다. 수집분 22개 문서 105개 회차로 확인했다 —
   * 월지급식은 전건이 이 꼴이고, 비월지급식은 이 칸의 수익률이 docMaxRate 와 전건 일치한다.
   *
   * (참여율형 ELB 3건(제4076~4078회)은 「가격상승률 × 참여율」 이라 만기 지급률이라는
   *  숫자 자체가 없다 — 여기서 안 잡히고 화면에서 「확인필요」로 남는 것이 맞다.)
   */
  const matRate = text.match(new RegExp(MAT_COND + '[\\s\\S]{0,140}?([\\d.]+)\\s*%\\s*\\(\\s*([^)\\n]{1,24}?)\\s*\\)'));
  p.maturityRate = matRate ? Number(matRate[2]) : null;
  p.maturityPayLabel = matRate ? clean(matRate[3]) : null;
  /* 「0%(원금지급)」 — 상환금액이 원금으로 확정된다는 표기 */
  p.maturityPrincipalOnly = !!(matRate && /원금|액면/.test(matRate[3]));
