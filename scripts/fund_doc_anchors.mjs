 ...
 *
 * ①②와 ③을 가르는 표는 **뒤에 오는 말**이다. 진짜 구간 첫머리는 반드시
 * 「1. 집합투자기구의 명칭」 으로 이어지고, 상호참조는 가리키려는 절 번호
 * (11. 매입 / 13. 보수)로 이어진다.
 *
 * 다만 제목만 덩그러니 놓인 속표지도 있으므로, 쪽을 그 제목으로 열면서 참조를
 * 뜻하는 말(참고·설명·바랍니다·페이지)이 뒤따르지 않으면 그것도 받는다.
 */
const P2_OPEN = /제\s*2\s*부[.\s]*집합투자기구에\s*관한(\s*사항)?/g;
const P2_FIRST_SECTION = /1\s*\.\s*집합투자기구의\s*명\s*칭/;
const P2_REFERS = /참\s*고|설명되어|바랍니다|페이지|참조/;
