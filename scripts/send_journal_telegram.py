...
두 가지를 보낸다.
  ...
  ② A4 한 장짜리 HTML 파일 — 받아서 인쇄하거나 브라우저로 열 수 있게
...
def main() -> int:
    ...
    html_path = f"docs/journal/{date}.html"
    ...
    if os.path.exists(html_path):
        ...
        ok, res = post(token, "sendDocument",
                       {"chat_id": chat,
                        "caption": f"국내 시장일지 {date} — A4 1장 + 대시보드 2장"},
                       filename=f"국내시장일지_{date}.html", fileblob=blob)
        print(("보냄" if ok else "실패") + " — HTML 첨부" + ("" if ok else f" · {res}"))
        fails += 0 if ok else 1
