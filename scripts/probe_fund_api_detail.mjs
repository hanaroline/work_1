...
for (const kw of KEYWORDS) {
  ...
  try {
    ...
    // ── 세 번 고쳤고 앞의 둘은 엉뚱한 데를 고쳤다 ────────────────────────
    // 첫 판은 `openapi|fileData|standard` 만 링크로 봤다. 안 걸리자 유형을
    // `[A-Za-z]+\.do` 로 넓혔고(16차), 그래도 안 걸리자 "연계 자료라 포털
    // 주소를 안 쓴다" 고 짐작해 닻을 전부 긁었다(17차). **둘 다 주소를
    // 의심한 고침이었고 둘 다 틀렸다** — 17차는 광주 연계 자료를 포털
    // 주소로 그냥 열었다.
    // 못 본 까닭은 제목이었다. 목록이 찾은 낱말을 강조 태그로 감싸므로
    // 태그를 지우면 `집합투자증권` 이 `집합 투자 증권` 이 된다. 아래에서
    // 빈칸을 떼고 견주는 한 줄이 진짜 고침이다. 닻을 전부 긁는 것은
    // 그대로 두되, 그것이 고친 것이라고 적어 두지는 않는다.
