 ...
 * 테스트판만 쪽 지도(ELS data/doc-pages.js · 펀드 data/fund-doc-pages.js)를
 * 싣는다. 앱은 그 파일이 없으면 쪽 표시를 아예 그리지 않으므로, 기존 배포본은
 * 동작이 달라지지 않는다.
 *
...
/* ① 쪽 지도를 싣는다 — 앱 스크립트보다 **먼저** 와야 한다 (앱이 로드 시점에 읽는다).
   아직 만들어지지 않은 지도가 있어도 여기서 멈추지 않는다. 앱은 없는 지도를
   빈 것으로 보고 그 상품에만 쪽 표시를 안 그린다 — 한쪽이 늦었다고 다른 쪽까지
   못 쓰게 만들 이유가 없다. */
...
console.log(`[test] ${OUT} 생성 — 쪽 지도 ${MAPS.join(' · ')} 적재 · 제목/상단바에 테스트 표시`);
