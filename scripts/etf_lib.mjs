...
/**
 * 원천이 **자리만 만들고 값을 안 채운 봉**을 찾는다.
 *
 * 2026-09-09 아침, 미국·홍콩·중국 ETF 의 기준일이 한 세션씩 뒤졌다. 야후에
 * 직접 물어 보니 09-08 봉이 분명히 있었다 — 그런데 그 봉의 종가가 null 이었다.
 * 자리는 만들어 두고 확정값은 아직 안 채운 것이다. computeReturns 는 값 없는
 * 봉을 건너뛰므로 결과는 맞았는데, "자료가 낡았나 우리가 잘못 받았나" 를
 * 가리는 데 반나절이 들었다. 원천이 무엇을 줬는지 그때 그 자리에 적어 두면
 * 다음에는 감사 보고서만 보면 된다.
 *
 * ⚠ null 을 Number() 에 넣으면 안 된다. Number(null) 은 0 이고
 *   Number.isFinite(0) 은 true 라, 빈 봉이 "값이 있는 봉" 으로 둔갑한다.
 *   이 파일의 다른 자리(편입비중 합계)에서 이미 한 번 당한 함정인데 여기서
 *   똑같이 밟았다 — 처음 판에서는 pendingBar 가 한 종목도 안 잡혔다.
 *
 * @returns {{day: string, count: number} | null}
 */
export function pendingBarOf(r) {
  const ts = r?.timestamp;
  if (!Array.isArray(ts) || !ts.length) return null;
  const cl = r?.indicators?.quote?.[0]?.close || [];
  const filled = (v) => v != null && Number.isFinite(Number(v));
  let i = ts.length - 1;
  while (i >= 0 && !filled(cl[i])) i -= 1;
  if (i === ts.length - 1) return null;            // 마지막 봉에 값이 있다
  return { day: new Date(Number(ts[ts.length - 1]) * 1000).toISOString().slice(0, 10),
           count: ts.length - 1 - i };
}
