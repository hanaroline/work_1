#!/usr/bin/env bash
# 수집한 데이터 폴더를 **데이터 전용 브랜치의 단일 커밋으로 갈아끼운다**(히스토리를 남기지 않는다).
#
# 왜 이렇게 하나
...
#   2) 이번 실행이 만든 $DATA_DIR 파일로 덮고
#   3) $DATA_DIR 만 담은 부모 없는 커밋을 만들어 --force-with-lease 로 올린다
#   4) 그 사이 다른 판이 올렸으면(lease 실패) 1)부터 다시 — 네 번까지
#
# 시장별 진입점(환경변수를 채워 이 스크립트를 부른다)
#   scripts/publish_us100_data.sh  ·  scripts/publish_kr100_data.sh
#
# 환경변수
#   DATA_DIR        올릴 폴더 (예: data/us100)                            [필수]
#   DATA_BRANCH     데이터 전용 브랜치 이름 (예: us100-data)              [필수]
#   PUBLISH_FILES   이번 실행이 만든 것 — 이것만 갈아끼운다               [필수]
#   KEEP_FILES      이 브랜치에 두어도 되는 파일 목록                     [기본: 아래]
#   COMMIT_LABEL    커밋 메시지에 붙는 설명 (예: 가격)                    [기본: 갱신]
#   COMMIT_TITLE    커밋 메시지 첫 줄의 대상 이름                          [기본: 데이터]
#   FETCHERS        커밋 메시지에 적는 수집기 경로                        [기본: 없음]

set -euo pipefail

BRANCH="${DATA_BRANCH:?DATA_BRANCH 를 지정해야 한다(데이터 전용 브랜치 이름)}"
SRC="${DATA_DIR:?DATA_DIR 를 지정해야 한다(올릴 폴더)}"
LABEL="${COMMIT_LABEL:-갱신}"
TITLE="${COMMIT_TITLE:-데이터}"

# 이 브랜치에 두는 것은 아래 넷뿐이다. 그 밖의 것(안내 파일 movedTo, REFRESH 트리거,
...
echo "올릴 것: $(cd "$MINE" && find . -type f | wc -l)개 파일 ($PUBLISH_FILES) → $BRANCH"
