...
/* 소스 화면 — 기본은 전체판. 상품설명의무 전용판은 --src 로 바꿔 넣는다.
   예) node scripts/build_sales_script.mjs sales-script-expl-standalone.html --src sales-script-expl.html */
const args = process.argv.slice(2);
const srcIdx = args.indexOf('--src');
const SRC = srcIdx >= 0 ? args[srcIdx + 1] : 'sales-script.html';
/* 내보낼 파일 이름 — 새 기능을 따로 받아 보려면 인자로 다른 이름을 준다.
   예) node scripts/build_sales_script.mjs sales-script-standalone-v3.html */
const OUT = args.find((a) => !a.startsWith('--') && a !== SRC) || 'sales-script-standalone.html';
