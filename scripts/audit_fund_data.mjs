...
for (const f of FUNDS) {
  ...
  // ── 자산구성 비중의 분모를 우리는 모른다 ────────────────────────────────
  //
  // 교보악사파워인덱스 2 에서 이런 관계를 찾았었다.
  //
  //   보유종목 비중 합 86.96% × (기준가 4000.71 / 1000) = 347.90%
  //   원천 자산구성 '주식'                                = 347.86%
  //
  // 0.05%p 차로 맞기에 "분모가 설정원본" 이라고 적었다. **전수에서는
  // 성립하지 않는다.** 보유종목이 전량 공시된 펀드(비중 합 90~110%)
  // 639개만 추려도 오차 5% 안에 드는 것은 44개(7%)뿐이다. 한 펀드에서
  // 맞은 것은 그 펀드에서 맞은 것이지 규칙이 아니었다.
  //
  // 보유종목을 거치지 않고 직접 봐도 마찬가지다. 자산구성 합은
  //
  //   합이 90~110  … 3,106개 중 681개(21.9%)   ← 순자산 대비
  //   합이 110 초과 … 2,416개                    ← 무엇 대비인지 모름
  //   기준가로 나눈 값이 90~110 … 358개(11.5%)  ← 설정원본 대비도 아님
  //
  // 로 갈린다. 한 자리에 두 가지 이상이 섞여 있고 어느 쪽인지 구별할
  // 근거가 없다. 그래서 **자산구성은 화면에 싣지 않는다.** 여기서는
  // 그 판단이 계속 유효한지만 센다 — 어느 날 원천이 정리되어 대부분이
  // 100 근처로 모이면 그때 다시 판단한다.
  //
  // 오류가 아니라 참고로 센다. 원천이 그렇게 주는 것이지 우리 자료가
  // 깨진 것이 아니고, 화면에 싣지 않으므로 사용자가 볼 일도 없다.
  if (f.assets && f.basePrice > 0) {
    const assetSum = Object.values(f.assets)
      .filter((v) => typeof v === 'number' && Number.isFinite(v))
      .reduce((a, b) => a + b, 0);
    if (assetSum > 0) {
      const perMil = assetSum / (f.basePrice / 1000);
      const near = (x) => x >= 90 && x <= 110;
      const kind = near(assetSum) ? '자산구성-순자산대비'
        : near(perMil) ? '자산구성-설정원본대비'
        : '자산구성-분모모름';
      flag('info', kind, f,
           `자산구성 합 ${assetSum.toFixed(2)}% · 기준가로 나누면 ${perMil.toFixed(1)}%`,
           { assetSum: +assetSum.toFixed(2), perMil: +perMil.toFixed(2) });
    }
  }
