...
// 처음 여는 사람이 **어느 칸이 그 칸인지**를 못 찾는다. "왼쪽 맨 앞 비교
// 칸" 이라고 적어도, 표를 처음 보는 사람에게는 그 칸이 어디 있는지가 안
// 보인다. 그래서 화면을 실제로 찍어 그 옆에 설명을 붙인다.
//
// 찍는 방식 —
...
${steps([
  ...
  `맨 왼쪽 <strong>비교</strong> 칸의 <strong>체크상자</strong>가 비교함에 담는 자리입니다. 표시하면 담기고 지우면 빠지며, 담긴 줄은 왼쪽에 주황 띠가 생깁니다. <u>표시하지 않아도 목록은 그대로 씁니다</u> — 담기는 비교할 때만 필요합니다. 최대 ${facts.maxCompare}개.`,
])}
...
${steps([
  ...
  `<strong>줄 맨 왼쪽 <u>비교</u> 칸에 표시하면 여기서 바로 담깁니다.</strong> 순위를 보다가 "이 둘을 견줘 보고 싶다" 가 될 때 이름을 외워 펀드 찾기로 돌아가지 않아도 됩니다 — 펀드 찾기에서 담은 것과 같은 바구니라 섞어 담아도 되고, 표 위 띠의 <strong>비교 탭에서 보기</strong> 로 바로 넘어갑니다. 최대 ${facts.maxCompare}개.`,
])}
...
${steps([
  ...
  '오른쪽 위 <strong>인쇄 · PDF 저장</strong> 은 <strong>지금 열어 놓은 탭 하나만</strong> 찍습니다. 비교 · 중복도를 보다가 누르면 그 화면만 나옵니다 — 여섯 탭이 통째로 딸려 나오지 않습니다. 검색창 · 거르개처럼 눌러야 하는 것들은 종이에서 할 일이 없어 빠집니다.',
])}
