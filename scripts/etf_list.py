...
# **종목 유형이 ETF 가 아닌데도 통과시키는 자리.** 아래 한 줄이 전부다.
#
# 첫 판에서 검산기가 GOLD.AX 를 막았다. 확인해 보니 종목은 맞았다 — 야후가 돌려준
# 이름이 'Global X Physical Gold Structured …' 로 표와 정확히 같고 통화는 AUD,
# 심볼은 `.AX` 다. 야후가 호주 상장 ETF 를 EQUITY 로 분류하는 것이 원인이었다.
#
# 그렇다고 유형 검사를 **경고로 낮추지 않는다.** 이 검사가 막으려던 바로 그 종목이
# 야후의 `GOLD`(바릭)이고, 바릭의 옛 이름은 'Barrick **Gold** Corporation' 이라
# 이름 검사만으로는 걸리지 않을 수 있다. 검사는 그대로 두고 **이 한 건만** 심볼까지
# 못박아 예외로 둔다 — 심볼이 다르면(맨 글자 GOLD 로 오면) 예외가 걸리지 않는다.
TYPE_EXCEPTIONS = {
    'GOLD': {'symbol': 'GOLD.AX', 'type': 'EQUITY',
             'why': ('야후가 호주(ASX) 상장 ETF 를 EQUITY 로 분류합니다. 이름·통화(AUD)·'
                     '심볼(.AX)이 모두 표의 펀드와 맞으므로 통과시킵니다 — 미국 상장 '
                     'GOLD(바릭, USD)가 아닙니다.')},
}
