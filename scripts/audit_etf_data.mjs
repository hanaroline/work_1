...
for (const e of ETFS) {
  ...
  // ── 2-1. 수익률이 아예 없는가 ───────────────────────────────────────────
  // 편입종목은 받아 왔는데 기간 수익률이 한 칸도 없는 종목이 있다. 화면은
  // "기간 수익률 자료가 없습니다" 라고 적으므로 거짓말은 아니지만, 목록·랭킹
  // 어디에도 안 서고 아무 규칙에도 안 걸려 **몇 종목인지조차 몰랐다**.
  // SPLG(미국 S&P500, 상장 10년이 넘는다) 가 여기 들어 있었다 — 새로 상장해
  // 이력이 없는 것과는 다른 얘기이므로 세어서 눈에 보이게 둔다.
  //
  // 잣대는 **화면과 같아야** 한다. 상세의 기간수익률 표는 총수익률(없으면
  // 시장가)만 보고 비었는지 정하므로, 기준가(nav)가 있어도 표는 안 나온다.
  // 여기서 nav 까지 세면 화면은 "자료 없음" 인데 감사는 "있다" 가 되어,
  // 두 잣대가 어긋난 채로 아무도 눈치채지 못한다.
  {
    const hasMain = [tr, price].some((o) => Object.values(o).some((v) => Number.isFinite(v)));
    if (!hasMain) {
      const navOnly = Object.values(nav).some((v) => Number.isFinite(v));
      flag('warn', '수익률-없음', e,
           `기간 수익률이 한 칸도 없다 — 상세에 표 대신 "자료 없음" 이 나간다 ` +
           `(편입종목 ${(e.holdings || []).length}개${navOnly ? ', 기준가 수익률만 있음' : ''})`,
           { holdings: (e.holdings || []).length, navOnly });
    }
  }

  // ── 2-1-1. 원천이 다른 종목 ─────────────────────────────────────────────
  // 야후 일봉이 비어 값을 못 만든 국내 소수 종목은 네이버 시장가 수익률로
  // 채웠다. 값이 있다고 조용히 넘어가면 "다른 원천·다른 기준" 이라는 사실이
  // 사라진다. 화면은 밝히고 있으니(retSource) 감사도 세어 둔다.
  if (e.retSource === 'naver') {
    flag('info', '수익률-네이버기준', e,
         `야후 일봉이 없어 네이버 시장가 수익률로 채웠다 — 총수익률이 아니다 ` +
         `(${Object.values(e.ret?.price || {}).filter((v) => v != null).length}칸, 기준일 ${e.retAsOf || '없음'})`,
         { periods: Object.values(e.ret?.price || {}).filter((v) => v != null).length });
  }
  ...
  // ── 8-0. 분배금을 못 찾은 총수익률 ─────────────────────────────────────
  // 총수익률은 야후 분배 이벤트로 만든다. 이벤트가 하나도 없으면 지수가
  // 시장가와 똑같아지고, 그 종목만 **분배금만큼 낮은 값**이 화면에 나간다.
  // 값이 깨진 게 아니라 조용히 낮은 것이라 다른 규칙에 걸리지 않는다.
  //
  // 가려내는 잣대는 공시 분배율이다. 분배율이 0 에 가까우면 tr 과 price 가
  // 같은 게 맞다(국내 122종목이 그렇고 대부분 정상이다). 분배율이 뚜렷한데
  // 내포분배율이 0 이면 그건 못 찾은 것이다 — 채권형에 몰려 있고, 네이버
  // 공표 총수익률보다 1.25~3.59%p 낮게 나왔다.
  if (Number.isFinite(e.dividendYield) && e.dividendYield >= 0.5
      && Number.isFinite(tr.Y1) && Number.isFinite(price.Y1)
      && Math.abs(tr.Y1 - price.Y1) < 0.005) {
    flag('warn', '분배금-못찾음', e,
         `분배율 ${e.dividendYield}% 인데 1년 총수익률(${tr.Y1}%)이 시장가(${price.Y1}%)와 같다` +
         ' — 원천에 분배 이력이 없어 그만큼 낮게 나온다',
         { dividendYield: e.dividendYield, tr: tr.Y1, price: price.Y1 });
  }
