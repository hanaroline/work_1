...
변동성도 달라진다. 환율이 주가와 같이 움직이면 더 커지고, 반대로 움직이면
상쇄된다. 미국 주식과 원/달러는 흔히 **반대로** 움직여(위험회피 때 달러가
강해진다) 원화 환산 변동성이 달러 기준보다 낮게 나오는 일이 많다. 그것이
고객이 실제로 겪는 변동성이므로, 낮게 나온다고 손보지 않는다.
