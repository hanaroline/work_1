...
def _appraised(prod, w):
    """기준가가 **시세가 아니라 감정평가**로 매겨진 펀드인가.

    비상장 실물 부동산·특별자산 펀드의 기준가는 거래된 값이 아니라 주기적
    감정평가다. 그래서 계열이 구조적으로 눌리고, 변동성·낙폭이 실제 위험보다
    훨씬 작게 나온다. 실제로 「이지스코어리테일부동산투자신탁 126」이
    **연4.7% · 변동성 0.69% · 최대낙폭 0%** 로 나와 위험 대비 6.8 이 되고,
    국내펀드 전체 1 위로 올라왔다. 숫자는 맞게 셈한 것이지만 **뜻이 틀렸다** —
    고객이 「거의 안 움직이는데 연 4.7%」로 읽으면 안 되는 상품이다.

    이름으로는 못 가린다. 같은 「특별자산」이라도 WTI원유는 38%, 금은 21%,
    리츠는 12.6% 로 멀쩡히 움직인다 — 그것들은 시세가 있기 때문이다.

    그래서 **값으로 가리되, 문턱은 자료가 알려 준 자리에 둔다.** 대체 노출
    상품의 연변동성을 줄 세우면 빈 구간이 뚜렷하다.

        0.69% · 1.93% · 2.12%   ← 비상장 실물 부동산 셋(감정평가)
        ───── 빈 구간 ─────
        6.40% · 7.82% · 8.08%   ← 여기부터는 시세가 있는 것들

    그 빈 자리인 **3%** 를 문턱으로 둔다. 처음에는 2% 로 두었는데, 같은
    운용사의 같은 성격 부동산펀드 둘이 문턱을 사이에 두고 갈렸다 — 하나는
    표시되고 하나는 1 위로 올라갔다. 자료에 없는 자리에 금을 그으면 그렇게 된다.

    가려낸 뒤 빼지는 않는다 — 멀쩡한 상품이다. 다만 위험 지표를 다른 상품과
    한 줄에 세우지 않고, 고객이 보는 줄에 그렇게 적는다.

    **노출은 여기서 직접 가린다.** 이 함수는 `reassign_by_exposure` 보다 먼저
    도는데, 처음에는 `prod["노출"]` 을 읽었다가 그때는 아직 비어 있어
    **채권형 셋이 「감정평가」로 잘못 표시됐다**(KCGI코리아채권·미래에셋코리아
    중기채권·삼성누버거버먼이머징단기채권). 셋 다 시세가 멀쩡히 있는 채권
    펀드이고, 변동성이 낮은 것은 채권이라서다.
    """
    e = EXP.exposure(prod) or {}
    big = max(e.items(), key=lambda kv: kv[1])[0] if e else "?"
    if big in ("국내채권", "해외채권", "현금성"):
        return False                      # 채권·현금은 원래 안 움직인다
    vol = w.get("vol")
    return isinstance(vol, (int, float)) and vol < 3.0
    ...
    for prod in products:
        ...
        if got and len(got.get("c") or []) >= 24:
            ...
            if w:
                prod["vol"] = w.get("vol")
                prod["mdd"] = w.get("mdd")
                if _appraised(prod, w):
                    prod.setdefault("flags", []).append("기준가_평가식_시세아님")
            n_series += 1
            continue
