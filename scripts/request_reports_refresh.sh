...
# 한 줄 고쳐 main 에 밀어 넣어 발동시킨다. (scripts/request_market_refresh.sh 와
# 같은 방식이다.)
#
#   bash scripts/request_reports_refresh.sh
#
# 작업 브랜치에서 실행해도 된다. 이 스크립트는 REFRESH 파일 하나만 별도로
# main 에 올리며, 현재 브랜치의 다른 변경은 건드리지 않는다.
set -euo pipefail
...
mkdir -p "$TMP/main/data/reports"
printf '%s\n' \
  '# 이 파일이 바뀌면 증권사 리포트 수집 워크플로가 돕니다. 내용은 쓰이지 않습니다.' \
  "요청: $STAMP (브랜치 $BRANCH)" > "$TMP/main/data/reports/REFRESH"
...
git -C "$TMP/main" add data/reports/REFRESH
...
git -C "$TMP/main" commit -q -m "리포트 수집 요청 $STAMP"

for wait in 0 2 4 8 16; do
  [ "$wait" -gt 0 ] && sleep "$wait"
  if git -C "$TMP/main" push -q origin main 2>/dev/null; then
    echo "요청 보냄: $STAMP"
    echo "수집은 보통 1~2분 걸린다. 끝나면 data/reports/latest.json 이 갱신된다."
    exit 0
  fi
