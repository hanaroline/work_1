...
#   push 이벤트는 밀어 넣은 브랜치에 놓인 워크플로 파일로 돈다. 그러니 지금
#   서 있는 브랜치로 보내면 된다. 아래에서 reports.yml 이 있는지 먼저 보는
#   까닭이 이것이다 — 없는 브랜치에 밀어 넣으면 아무것도 돌지 않는다.
#
#   예전 판은 시세 쪽 스크립트를 본떠 main 을 따로 클론해 거기에 올렸는데,
#   그때 main 에는 이 워크플로가 없어 아무것도 돌지 않았다(9/8 08:03 에 확인).
