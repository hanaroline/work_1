...
// **요약본을 없앴다**(2026-09-12). 상세를 접어 인쇄하면 종이에는 펼칠 단추가
// 없어 제목만 찍히고 본문이 사라진다 — 근거 블록 일곱이 그렇게 나가
// 「빠진 자리」로 읽혔다. 펼쳐서 뽑으면 전체본과 한 글자도 다르지 않으니
// 같은 파일을 이름만 달리해 둘 보내는 셈이 된다.
//
// 짧은 판이 필요하면 **접어서 감추는 것이 아니라 핵심본을 쓴다** — 같은
// 내용을 나란히 앉혀 훨씬 적은 쪽수에 담는다(지침 7-0절).
const JOBS = DENSE ? [[true, '']] : [[true, '_전체']];
