...
def main():
        ...
        "items": [
            {
                ...
                # 과세비율. 못 구한 종목은 null 로 둔다 — 1 로 적으면 '실제로
    ...
    # 세금 문구는 **실제로 확보된 만큼만** 말한다. 한 종목도 못 구했는데
    # "종목마다 다른 비율을 적용했습니다" 라고 적으면 그 자체가 거짓말이 된다.
    _have = [x for x in selectable if x.get("taxableRatio") is not None]
    _cov = len(_have) / len(selectable) if selectable else 0
    _why = ("세금은 분배금 전액이 아니라 <b>과세표준액</b>에만 붙습니다. 국내주식 매매차익과 장내파생 손익은 "
            "과세표준에 들어가지 않으므로, 그 재원으로 분배하는 종목은 분배금의 상당 부분이 비과세입니다.")
    _how = ("과세비율은 <b>최근 12개월에 실제로 매겨진 과세표준액을 같은 기간 분배금으로 나눈 값</b>입니다. "
            "지나간 실적이므로 앞으로도 같다는 뜻은 아닙니다 — 분배 재원(배당·이자·매매차익·파생손익)의 구성이 "
            "바뀌면 비율도 함께 바뀌고, 실제로 한 종목 안에서도 회차마다 0%에서 17%까지 움직인 사례가 있습니다. "
            "실제 세액은 지급 시점의 과세표준으로 확정되므로 이 문서와 다를 수 있습니다.")
    if _cov >= 0.5:
        _tax = [f"{_why} 그래서 종목마다 다른 과세비율을 적용했습니다"
                f"(고를 수 있는 {len(selectable)}종목 중 {len(_have)}종목).",
                _how + " 과세비율을 확인하지 못한 종목은 분배금 전액에 세율을 적용해 보수적으로 계산했으므로, "
                "실제 세후 수령액이 표시된 금액보다 많을 수 있습니다."]
    elif _have:
        _tax = [_why,
                f"다만 이번 자료에서는 고를 수 있는 {len(selectable)}종목 중 <b>{len(_have)}종목만</b> 과세표준을 "
                "확인할 수 있었습니다. 나머지 종목은 정확한 과세표준을 확인하지 못해 <b>부득이 분배금 전액에 "
                "15.4%를 적용</b>했습니다. 그 종목들의 실제 세후 수령액은 표시된 금액보다 많을 수 있습니다.",
                _how]
    else:
        _tax = [_why,
                "다만 이번 자료에서는 <b>종목별 과세표준을 확인하지 못했습니다.</b> 그래서 부득이 모든 종목의 "
                "분배금 전액에 15.4%를 적용해 계산했습니다. 이는 세금을 가장 많이 매기는 가정이므로, 실제 세후 "
                "수령액은 이 문서에 표시된 금액보다 많을 수 있습니다. 특히 국내주식형 커버드콜처럼 매매차익·"
                "파생손익으로 분배하는 종목은 차이가 큽니다."]
    html = html.replace("__TAXNOTES__",
                        "\n".join(f"      <li>{t}</li>" for t in _tax))
...
function recalc() {
  ...
  // 과세표준을 확인하지 못한 종목은 전액 과세로 셈한다. 그 사실을 말하지
  // 않으면 세후 금액이 실제보다 적게 나온 것을 아무도 모른다.
  $('wTax').textContent = cs.some(c => c.it.taxR === null || c.it.taxR === undefined)
    ? '※ 과세표준을 확인하지 못한 종목이 담겨 있습니다. 그 종목은 분배금 전액에 세율을 적용해 계산했으므로, 실제 세후 수령액은 표시된 금액보다 많을 수 있습니다.' : '';
