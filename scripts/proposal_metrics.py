...
def components(p, rf):
    ...
    # **기준가가 시세가 아닌 상품은 위험 항목을 쓰지 않는다.** 비상장 실물
    # 부동산·특별자산의 기준가는 주기적 감정평가라 계열이 눌린다. 그대로
    # 두면 「연4.7% · 변동성 0.69% · 낙폭 0%」가 위험 대비 6.8 이 되어 국내펀드
    # 1 위로 올라온다 — 셈은 맞지만 뜻이 틀렸다. 잴 수 없는 것을 잰 척하지
    # 않고, 보수·수익·일관성만으로 줄을 세운다.
    if "기준가_평가식_시세아님" in (p.get("flags") or []):
        out["risk_adj"] = None
        out["vol"] = None
        out["mdd"] = None
        mark = " (감정평가 기준가 — 시세가 아니라 위험을 못 쟀습니다)"
