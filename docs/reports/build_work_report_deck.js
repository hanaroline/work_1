...
function featureCard(slide, o) {
  ...
  const c = o.compact;
  const pad = c ? 0.22 : 0.26;
  chip(slide, o.x + pad, o.y + (c ? 0.2 : 0.24), o.n, o.accent, c ? 0.38 : 0.44);
  slide.addText(o.title, {
    x: o.x + pad + (c ? 0.5 : 0.56), y: o.y + (c ? 0.21 : 0.26),
    w: o.w - pad * 2 - (c ? 0.5 : 0.56), h: 0.34, isTextBox: true, margin: 0,
    fontFace: F, fontSize: c ? 12.5 : 14.5, bold: true, color: INK,
  });
  slide.addText(o.body, {
    x: o.x + pad, y: o.y + (c ? 0.68 : 0.76), w: o.w - pad * 2,
    h: o.h - (c ? 1.2 : 1.36), isTextBox: true, margin: 0,
    fontFace: F, fontSize: c ? 10 : 11, color: BODY, lineSpacing: c ? 14 : 15.5,
  });
  slide.addText(o.metric, {
    x: o.x + pad, y: o.y + o.h - (c ? 0.46 : 0.5), w: o.w - pad * 2, h: 0.28,
    isTextBox: true, margin: 0,
    fontFace: F, fontSize: c ? 9.5 : 10.5, bold: true, color: ORANGE_DEEP,
  });
}
   ...
   ══════════════════════════════════════════════════════════════ */
{
  ...
  s.addText("매일·매주 도는 자동 산출물 4종 · 현장 업무 화면 10종 · 자동화 워크플로 35종", {
    x: M, y: 3.5, w: 7.3, h: 0.86, isTextBox: true, margin: 0,
    fontFace: F, fontSize: 16, color: W, lineSpacing: 25,
  });

  [
    ["구축 기간", "   2026.06 – 09.10  (약 4개월, 운영 계속)"],
    ["수행 방식", "   Claude Code 세션 48회 + GitHub Actions 예약 실행"],
    ["산출물 위치", "   사내 저장소 work_1 — 작업 브랜치 39개"],
  ].forEach(([k, v], i) => {
    s.addText(
      [
  ...
  [
    ["14", "종", "현장에서 쓰고 있는 화면·자동 산출물"],
    ["35", "종", "무인 운영 중인 수집·검증 워크플로"],
    ["64", "판", "자동 발행한 시황 브리핑 (33일 연속)"],
    ["4,700", "여", "조회 가능한 상품·종목 (펀드·ETF·ELS·주식)"],
  ...
  s.addNotes(
    ...
    "작업은 세션 48회, 저장소 작업 브랜치 39개로 남아 있습니다. " +
    "성과는 세 갈래입니다 — (1) 매일·매주 스스로 도는 자동 산출물 4종, " +
    "(2) 상품 판매와 고객 잔고를 다루는 현장 업무 화면 4종, " +
    "(3) 시세·상품 조회 화면 6종. 전부 지금 쓰고 있는 것들입니다. " +
    "지난 이틀 사이에도 국내 100대 기업 화면과 증시 일정 캘린더가 붙었고, 완전판매는 펀드까지 넓혔습니다."
  );
}
   ...
   ══════════════════════════════════════════════════════════════ */
{
  ...
  const phases = [
    ...
    ["3단계", "8 ~ 9월", "현장 업무 화면 구축", "상품 판매·고객 잔고·시세 조회를 설치 없는 단일 화면으로 묶었습니다. 지금도 화면이 늘고 있습니다."],
  ];
  ...
  s.addTable(
    [
      ...
      ["시세·상품 조회", "미국 100대 기업 · 국내 100대 기업 · 증시 일정 캘린더 · ELS 상품 조회 · 국내 종목 통합 리포트 · 마포WM 모바일 창구", "단일 파일 배포"],
  ...
  footnote(
    s,
    "세션 기록으로 확인되는 작업은 7월 9일부터 48회입니다. 6월은 도구를 익히던 시기로 별도 산출물 기록이 남아 있지 않습니다. 9월 9~10일에 국내 100대 기업·증시 일정 캘린더가 더해졌습니다."
  );
   ...
   ══════════════════════════════════════════════════════════════ */
{
  ...
  s.addText("8/31 주부터 핵심본·전체본 두 판을 함께 냅니다. 9/07 주는 4일치입니다.", {
  ...
  s.addTable(
    [
      ...
      ["시세 신선도 확인", "지난 거래일 값 · 덩어리째 빠진 스냅숏"],
  ...
  footnote(
    s,
    "브리핑 64판 구성 — 모닝 25판 · 장마감 14판 · 핵심본 17판 · 해외 증시(주말·휴장일) 8판. 검증 중 하나라도 어긋나면 발행을 멈추고 사유를 남깁니다."
  );
   ...
   ══════════════════════════════════════════════════════════════ */
{
  ...
  const cards = [
    {
      ...
      body: "미스터리쇼퍼 대응용 스크립트를 상품별로 자동 완성합니다. 상품을 고르면 투자설명서 항목이 문장에 주입되고, 큰 글씨 프롬프터로 읽으며 셀프 채점까지 이어집니다. 설명의무만 남긴 전용판을 9월에 더했고, ELS 에 이어 펀드까지 넓혔습니다.",
      metric: "평가표 8종 × 16조합 · 펀드 설명서 3,019건 판독 · 매일 10:10 갱신",
    },
    {
  ...
  footnote(
    s,
    "9월 갱신 — 펀드 투자설명서 판독기를 다시 써 3,014건을 재판독했습니다. 수수료율을 못 읽던 펀드 479건 → 84건, 근거 없는 「환매수수료 부과」 문구 222건 → 0건."
  );
   ...
   ══════════════════════════════════════════════════════════════ */
{
  const s = pres.addSlide();
  s.background = { color: W };
  sectionHead(s, "시세·상품 조회 화면 6종 — 설치 없이 파일 하나로", "성과 ③");

  const cards = [
    {
      n: "1", accent: ORANGE, title: "미국 100대 기업",
      body: "대형주 100개를 검색·비교·차트·컨센서스·실적 일정까지 10개 섹션으로. 장중 10분 주기로 가격이 스스로 갱신됩니다.",
      metric: "100개 기업 · 10개 섹션 · 4종목 비교",
    },
    {
      n: "2", accent: BLUE, title: "국내 100대 기업",
      body: "유가증권 94 · 코스닥 6. 미국 화면과 같은 구조·조작으로 만들어 두 시장을 같은 눈으로 봅니다. 오프라인 판을 따로 냅니다.",
      metric: "100개 기업 · 실시간판 + 오프라인판",
    },
    {
      n: "3", accent: ORANGE, title: "증시 일정 캘린더",
      body: "경제지표 발표·중앙은행 결정·실적발표·제약바이오 학회·파생 만기·휴장일을 한 화면에. 관심 일정은 ICS 로 내려받아 캘린더에 담습니다.",
      metric: "10개 섹션 · 547건 · ICS·CSV 내보내기",
    },
    {
      n: "4", accent: BLUE, title: "ELS 상품 조회",
      body: "청약 진행 상품을 매일 자동 수집해 기초자산·조기상환 조건·수익 구조를 한 화면에 놓고, 만기 손익을 시뮬레이션합니다.",
      metric: "37개 상품 매일 수집 · 손익 시뮬레이터",
    },
    {
      n: "5", accent: ORANGE, title: "국내 종목 통합 리포트",
      body: "종목명만 넣으면 시세·기술적지표·외국인/기관 수급·증권사 목표주가·뉴스·재무제표를 한 장으로 묶어 보여줍니다.",
      metric: "6개 섹션 · PDF 저장 · 최대 4종목 비교",
    },
    {
      n: "6", accent: BLUE, title: "마포WM 모바일 창구",
      body: "전단의 QR 13개를 링크 한 개로 옮겨, 고객이 문자로 받은 주소를 누르면 해당 업무가 바로 열립니다. 배포는 자동입니다.",
      metric: "13개 업무 · QR 스캔 단계 제거",
    },
  ];
  cards.forEach((c, i) => {
    featureCard(s, {
  ...
  // 그 외 단건 산출물
  s.addShape(pres.ShapeType.roundRect, {
    x: M, y: 5.96, w: CW, h: 0.82, rectRadius: 0.03,
  ...
  s.addText(
    ...
    { x: M + 0.3, y: 6.06, w: CW - 0.6, h: 0.64, isTextBox: true, margin: 0, fontFace: F, lineSpacing: 15 }
  );

  footnote(
    s,
    "여섯 화면 모두 빌드 도구도 설치도 없이 파일 하나로 동작합니다. 국내 100대 기업과 증시 일정 캘린더는 9월 9~10일에 더한 것입니다."
  );
   ...
   ══════════════════════════════════════════════════════════════ */
{
  ...
  s.addTable(
    [
      ...
      ["환매수수료 오판독", "판독기 재작성 후 근거 없는 문구 0건"],
      ["지어낸 요약", "리포트 원문 문장만 골라 실음"],
    ],
    {
      x: 7.2, y: 3.16, w: 5.2,
      colW: [1.86, 3.34],
      rowH: [0.32, 0.36, 0.36, 0.36, 0.36, 0.36, 0.36],
      fontFace: F, fontSize: 10.5, color: BODY, valign: "middle",
      border: { type: "solid", color: "E5E4E1", pt: 0.75 },
      margin: [0.04, 0.1, 0.04, 0.1],
    }
   ...
   ══════════════════════════════════════════════════════════════ */
{
  ...
  const rows = [
    ["14종", "현장에서 쓰고 있는 화면·자동 산출물 (그 외 단건 다수)"],
    ["35종", "무인 운영 워크플로 — 예약 20종 · 장중 10분 주기 갱신"],
    ["64판 / 33일", "시황 브리핑 무인 발행 — 휴장일 포함 무결점 연속"],
    ["약 64시간", "브리핑 작성 시간 절감 (추정 · 판당 60분 기준)"],
    ["4,777", "커버리지 — 펀드 3,192 · ETF 1,348 · 미국 100 · 국내 100 · ELS 37"],
    ["39개", "작업 브랜치 — 산출물·수집·검증이 저장소에 남음"],
  ];
  rows.forEach(([k, v], i) => {
    const y = 2.14 + i * 0.74;
    s.addText(k, {
  ...
  [
    ...
    ["완전판매 — 채권·IRP 남음", "ELS 에 이어 펀드까지 붙였습니다(9월). 채권·IRP 설명서를 마치면 전 상품군이 자동완성됩니다."],
  ...
  footnote(
    s,
    "기준일 2026.09.10.  절감 시간은 시세 취합·작성·검증을 사람이 할 때 판당 60분으로 보고 발행 64판에 곱한 추정치이며, 실측값이 아닙니다. 그 외 수치는 저장소 산출물과 세션 기록에서 직접 집계했습니다.",
    true
  );

  s.addNotes(
    "정량 성과에서 유일한 추정치는 절감 시간입니다. 판당 60분으로 보수적으로 잡아 약 64시간이며, 실측이 아니라고 명시했습니다. " +
