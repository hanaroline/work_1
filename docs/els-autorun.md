...
2. **역추적 검증** — 렌더된 PDF의 숫자 하나하나가 대장까지 거슬러 올라가는지 본다.
   ...
   **손으로 적어 둔 숫자를 잡아내는 건 이 검사뿐이다.** 발표판에 경로 수가
   ...
   인쇄에서 뺀다.
3. **표가 아래 안내 상자를 깔고 앉지 않았는지** 본다. 2장 표는 행 수에 맞춰 높이를
   나누는데, 머리글이 한 줄 늘거나 기초자산 이름이 길어 행이 접히면 표가 내려와
   마지막 회차가 안내 상자 밑으로 들어간다. 텍스트 bbox 검사로는 안 잡힌다 —
   슬라이드 안에는 있기 때문이다. 표 마지막 글자의 y 와 안내 상자의 y 를 직접 견준다.
4. **빈값 누출 검사** — 두 PDF 본문에 `null` · `undefined` · `NaN` 이나 값 자리의
   `–` 가 없는지 본다. 기초자산이 하나뿐이거나(상관계수 없음) 낙인이 없는 상품이
   섞이면 여기서 샌다. 실제로 "그 선이 null%로" 가 인쇄된 적이 있다(2026-09-21).
5. **두 문서 대조** — 추천 3종·주의 4종·핵심 수치가 덱과 분석자료에서 같은지 본다.
   둘 다 `analyze()` 에서 나오므로 어긋나면 어느 한쪽이 값을 다시 계산한 것이다.
6. **1페이지 추천 1순위 = 4페이지 추천 3종의 첫 칸**인지 확인한다. 예전에
   두 곳이 서로 다른 상품을 가리킨 적이 있다.
7. 온라인 전용 회차 표시가 붙었는지 확인한다. 판정 출처는 `rendered_list.json`
...
- **5장 산점도는 계열을 회차마다 하나씩 둔다.** 파워포인트가 점 위에서 보여 주는
  풍선말은 계열 이름이라, 추천/그 외/주의 세 계열로 묶으면 정작 어느 회차인지가
  안 뜬다(2026-09-21 요청). 대신 자동 범례를 쓸 수 없으므로 세 가지 색을 손으로
  적고, 안내 문구는 부제에 붙인다 — 범례 옆 자리는 좁아 세 줄로 접힌다.
