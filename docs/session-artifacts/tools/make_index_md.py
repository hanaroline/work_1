...
L.append('- 기준 2026-09-24 · 저장소 `hanaroline/work_1`')
L.append('- 세션 59개 · 주제 %d개 · 색인 항목 %d개 · 이 자료실에서 만들어 올린 아티팩트 %d개\n'
...
L.append('- 시장 팩트시트 2건은 아티팩트 목록에서 찾았으나, 어느 세션이 만든 것인지는 '
         '세션 목록으로 확인되지 않았습니다.')
L.append('- `국내 설정 공모펀드 조회` 는 단일 파일이 28.5MB 라 아티팩트 상한(16MB)을 '
         '넘습니다. 저장소에서 파일을 내려받아 열어야 합니다.')
L.append('- 페이지가 파일을 건네주는 단추(HTML·CSV·PDF 저장)는 아티팩트에서 그냥은 '
         '동작하지 않습니다. 발표자료 페이지와 `ETF 편입종목 조회` 는 `downloads` 를 '
         '선언해 두어 눌리지만, `완전판매 스크립트` 의 저장 단추는 아직 먹지 않습니다.')
L.append('- `ETF 편입종목 조회` 는 8/31 부터 9/10 까지 **빈 화면**이었습니다. '
         '아티팩트용 조각을 뽑는 스크립트가 본문을 `</body>` 로 잘라내는데, 8/31 에 '
         '들어온 함수가 자바스크립트 문자열 안에 `</body>` 를 담고 있어 마지막 '
         '`<script>` 가 닫히지 않은 채 끊겼습니다. 브라우저는 그 조각을 오류 없이 '
         '통째로 버립니다. 9/10 에 고쳐 다시 올렸고, 고친 생성기는 '
         '`tools/build_etf_artifact.mjs` 에 있습니다. 그때까지 이 링크를 받은 분께는 '
         '다시 열어 보시라고 알려야 합니다.')
L.append('- 9/24 기준으로 `국내·미국 100대 기업`·`증시 일정 캘린더`·'
         '`증권사 리포트 다이제스트`·`완전판매 스크립트`(전체판) 는 저장소에 더 새 판이 '
         '있지만 아직 아티팩트를 갈아 끼우지 못했습니다. 아티팩트를 덮어쓰려면 지금 '
         '올라가 있는 판을 통째로 다시 읽어야 하는데, 이 화면들은 자료를 파일에 담고 '
         '있어 한 건이 6,000줄 안팎입니다. 각 화면이 자기 자료의 시점을 스스로 밝히므로 '
         '틀린 값이 나가지는 않습니다.')
L.append('- `TV · 라디오 온에어` 는 아티팩트로 올릴 수 없습니다. 유튜브 임베드·방송 '
         '스트림·채널 목록이 모두 외부 요청이라 아티팩트 안에서는 빈 화면이 됩니다. '
         '저장소를 내려받아 로컬 서버로 열어야 합니다.')
L.append('- `월배당 ETF 고객제안서` 는 엑셀 파일이라 아티팩트 페이지로 열 수 없습니다. '
         '저장소 `claude/intelligent-maxwell-dnreiw` 에서 받으십시오.')
L.append('- `은퇴자산 운용 제안서` 는 특정 가구의 자산 내역이 담긴 개별 제안서입니다. '
         '이름·연락처는 없지만 총자산·은퇴시점·부부 금융소득이 그대로 있어 당사자를 '
         '아는 사람에게는 식별됩니다. 링크를 상담 관계 밖으로 돌리지 마십시오.')
L.append('')
