...
앱을 띄우는 것과 **그 전략이 먹히는지**는 다른 물음이다. 후자는 이 저장소가 이미 모아 둔
일봉으로 답할 수 있다 — KIS 서버가 필요 없다. 그 판이
[`docs/kis-timing/index.html`](kis-timing/index.html) 이다.

기본 전략 10종을 국내주식 100 · 미국주식 99 · 국내ETF 48 · 해외ETF 18 에 대고 재고,
학습/검증 구간을 나눠 **어느 전략이 어느 시장에서 서는지** 가린 뒤, 오늘 무엇을 얼마에
사고 어디서 자를지까지 낸다. 요약하면 —

- 10종을 그대로 쓰면 안 된다. 시장마다 **넷~여섯 종**만 남았다.
- 신호 하나로는 초과수익이 거의 없다. **둘 이상 겹칠 때**만 의미가 생겼다.
- **미국 대형주에서는 서지 않았다** (검증구간 −0.64%p).
- **「돌파 실패」는 반대로 나왔다** — 매도 신호인데 신호 뒤가 늘 대조군보다 좋았다.

자세한 수치와 돌리는 법은 README 의 「전략빌더 10종 — 매매 타이밍」 절에 있다.
