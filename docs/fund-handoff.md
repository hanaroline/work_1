# 펀드 화면 — 인수인계

> **2026-08-29 갱신.** 이 문서가 넘긴 작업은 끝났습니다. 수집기·빌드·감사·
> 화면·워크플로가 모두 있고, 하나 남아 있던 미해결 항목(244.9%)도 풀렸습니다
> ([아래](#아직-안-풀린-것--풀렸습니다-2026-08-29)). 만들어진 것은 README 의
> "국내 설정 공모펀드 조회" 절에 있습니다. 아래 본문은 **출발 당시의 기록**
> 그대로 두었습니다 — 무엇을 알고 시작했는지가 남아 있어야 하기 때문입니다.

ETF 화면(`etf.html`)과 같은 구성으로 **국내 설정 공모펀드** 화면을 만드는
작업의 출발점입니다. 원천 조사는 끝났고 수집기·화면은 아직 없습니다.

이 문서는 ETF 세션에서 확인한 것만 적습니다. **짐작은 적지 않았습니다.**
확인 경로는 `tools/discovery/fund_probe*.md`, `fund_naver*.md` 에 있습니다.

---

## 범위

사용자가 정한 것 그대로입니다.

> "국내 설정 펀드를 투자지역으로 나누는 것임"

곧 **국내에 설정된 공모펀드를, 투자 지역(국내/해외)으로 가른** 화면입니다.
해외에 설정된 뮤추얼펀드가 아닙니다.

---

## 원천 — 네이버 Npay 증권

`stock.naver.com` 입니다. `finance.naver.com/fund/` 는 죽었습니다.

> **주의.** 옛 주소가 일반 증권 홈으로 넘어가는 것을 보고 "펀드 서비스가
> 없어졌다" 고 단정해 금투협을 여섯 차례 헛팠습니다. 사용자가 화면 사진을
> 주어 바로잡혔습니다. 넘어간 곳만 보고 단정하지 마십시오.

### 목록

```
GET https://stock.naver.com/api/fund/funds?page={0..159}&size=20
```

- **`size` 상한이 20 입니다.** 50·100 은 HTTP 400 입니다.
- 총 **3,196 개**, 160 페이지. 응답 meta 에 `totalCount`·`hasNext` 가 옵니다.
- `sort=return3m` 같은 정렬 인자를 받습니다.
- 주는 것: `fundCode`(금투협 표준코드, 예 `K55235B39916`), `fundName`,
  `riskGrade`, `nav`, `basePrice`, `returnIndex`, `returnRate1m/3m/6m/1y/3y/5y`,
  `tradeDate`, `changePrice`, `totalFee`, `preSalesFee`, `postSalesFee`,
  `pcUrl`, `mobileUrl`
- **없는 것: 유형·운용사·설정액.** 그래서 펀드마다 `left-panel` 을 또 불러야 합니다.

### 상세 (모두 브라우저 없이 재현됨)

```
GET https://stock.naver.com/api/fund/funds/{fundCode}/left-panel
                                          /fund-performance
                                          /chart-price-panel
                                          /base-price/chart
                                          /prices/daily
                                          /metrics/detail
                                          /classes/returns
```

`left-panel.detail` 의 주요 필드:

```json
{ "fundCode": "K55235B39916",
  "fundName": "피델리티글로벌테크놀로지증권모투자신탁(주식-재간접형)",
  "basePrice": "5956.79", "returnIndex": 0.2365916,
  "parentPeerGroupName": "해외주식형",          // ← 유형
  "derivedAum": "774093701808",                // 설정액
  "derivedNav": "4611116881228",               // 순자산
  "inceptionDate": "2015-06-17",
  "companyName": "피델리티자산운용",
  "benchmarkName": "MSCI AC WORLD INDEX (KRW Unhedged)",
  "totalFee": null, "managementFee": null, "salesFee": null }
```

`fund-performance` 는 **ETF 에 없던 것**을 줍니다 — 기간마다 세 값입니다.

```json
"returns": [
  { "term": "1m", "fundReturn": 7.249547,
    "benchmarkReturn": -1.9567338, "peerCompanyReturn": null }
]
```

`metrics/detail` 은 표준편차·추적오차·샤프·정보비율·젠센알파·베타를 줍니다
(`fundMetric`). 옆에 `peerMetric` 자리가 있으나 표본에서는 `null` 이었습니다.

### 보유종목

```
chart-price-panel.allocationsPortfolio.result[]
  -> { itemCode(ISIN), itemName, weight(소수) }
```

`availability` 가 유형별로 갈립니다. 60종목 표본:

| 유형 | 표본 | 보유종목 있음 | 종목수(중앙) |
|---|---:|---:|---:|
| 국내주식형 | 9 | **9 (100%)** | **72** |
| 국내혼합형 | 11 | **10** | 44 |
| 해외주식형 | 16 | **13** | 40 |
| 해외혼합형 | 8 | 5 | 38 |
| 기타형 | 6 | 1 | 2 |
| 해외채권형 | 2 | 1 | 1 |
| 국내채권형 | 6 | **0** | – |
| MMF | 1 | **0** | – |
| 국내대체 | 1 | 1 | 196 |

**주식형·혼합형은 나오고 채권형·MMF 는 없습니다.** 재간접형이 비는 것도
당연합니다 — 다른 펀드를 담으니 개별 종목이 없습니다.

ETF 는 상위 10개만 받았지만 펀드는 **전체 보유종목**(40~72개)을 줍니다.

### 안 되는 것 → **절반만 맞았습니다** (2026-08-29 정정)

원래 적혀 있던 것:

> **총보수를 못 받습니다.** 60종목 중 59종목에서 `totalFee: null` 입니다.
> 목록·상세 모두 그렇습니다. 보수 비교는 이 원천으로 못 만듭니다 —
> 화면에서 빼거나 다른 원천을 찾아야 합니다.

**펀드 단위로는** 맞습니다. `detail.totalFee` 는 거의 다 null 입니다.

그런데 `left-panel` 의 `returns.classes[]` 에는 **클래스별 총보수가 채워져
있습니다** — 전수 수집에서 클래스 19,092개 중 **18,712개(98%)**, 펀드로는
3,154/3,196개입니다.

국내 공모펀드의 보수는 원래 클래스마다 다릅니다. 신한골드증권투자신탁 1 은
클래스가 18개이고 종류C-pe 가 1.04%, 종류C-p 가 1.39% 입니다. 전수에서
**2,885개 펀드가 클래스마다 보수가 다릅니다.**

그래서 하나의 숫자로 줄이지 않고 **최저~최고 범위**로 싣고 클래스별 표를
함께 둡니다.

> **여기서 얻은 것.** "없다" 는 판정도 **어느 자리에서 없는지**까지 적어야
> 합니다. `detail.totalFee` 가 비어 있는 것을 보고 "이 원천에는 총보수가
> 없다" 로 넘겼는데, 같은 응답의 한 단계 아래에 있었습니다. 화면을 다 만들고
> 파일 크기를 재다가 `classes` 가 2.66MB 인 것이 눈에 띄어 열어 보고서야
> 알았습니다.

---

## 아직 안 풀린 것 → **풀렸습니다** (2026-08-29)

원래 적혀 있던 것:

> **단기채권 펀드의 1개월 수익률이 244.9% 로 옵니다.**
> (골든브릿지으뜸단기증권투자신탁 1[채권], 기준가 3,361.5)
>
> 처음에는 "목록이 설정 이후 누적을 준다" 고 의심했는데 **틀렸습니다** —
> 목록과 상세를 16종목 대조하니 **16/16 완전 일치**했습니다. 즉 원천 불일치가
> 아니라 네이버가 두 곳에 같은 값을 싣고 있습니다.
>
> 기준가 3,361.5 는 설정 1,000 기준 3.36배이고 244.9% 와 가깝습니다. 일부
> 펀드에 설정 이후 누적이 실릴 가능성이 남아 있습니다.

**설정 이후 누적이 아니었습니다.** 기준가 계열의 **재산정 계단**이었습니다.
검산 1~4차의 기록은 `tools/discovery/fund_returns_verify{,2,3,4}.md` 에,
스크립트는 `scripts/verify_fund_returns{,2,3,4}.mjs` 에 있습니다.

### 1차 — 원천은 자기 계열과 앞뒤가 맞는다

기준가 시계열에서 기간수익률을 다시 계산하니 원천과 **소수 넷째 자리까지
일치**했습니다(1개월 30/31). 곧 244.939% 는 네이버가 어디선가 잘못 가져온
값이 아니라, 네이버 자신의 기준가 계열에서 그대로 나오는 값입니다.

### 2차 — 계단의 실물

```
2026-07-31    974.57    그날 등락률 +0.002%
2026-08-03   3356.48    그날 등락률 +244.406%
```

하루 0.002% 씩 움직이던 단기채권 펀드의 기준가가 하루 만에 3.44배가 됐습니다.
네이버 자신도 `prices/daily` 에 그날 등락률을 244.4% 로 적습니다. 곧 원천은
앞뒤가 맞고, 다만 그 값이 수익률이 아니라 **재산정**일 뿐입니다.

### 3차 — 탐지 규칙이 틀렸고, 하필 놓치는 쪽으로 틀렸다

2차의 규칙(배율을 하루당으로 환산해 1.5배)이 골든브릿지스마트단기채
(3.147배)를 **놓쳤습니다.** 사이에 주말이 끼어 사흘로 나뉘자
3.147^(1/3) = 1.466 이 되어 문턱 아래로 내려간 것입니다. 주말은 사흘어치
복리가 아닙니다. 그대로 두었으면 화면에 **+215%** 가 나갔습니다.

고친 규칙은 보편 상수 대신 **그 펀드 자신의 평소 폭**(로그수익률 MAD)과
견줍니다. 절대 바닥과 평소 폭의 8배를 **모두** 넘어야 계단으로 봅니다.

### 4차 — 계단이 있다고 무조건 비우면 멀쩡한 값을 버린다

3차가 703종목에서 21종목을 잡았는데 18종목은 원천 수익률의 앞뒤가
멀쩡했고, 그 계단은 거의 다 **아래로** 났습니다(0.23~0.69배). 결산·분배로
기준가를 1,000 으로 되돌리는 것이고, **네이버는 그 계단을 보정합니다.**

```
KB연금미국S&P500  1년   원천 +17.49%   생값 −60.12%
```

계단만 보고 비웠으면 이 펀드에서만 멀쩡한 칸 5개를 버렸습니다.

**그래서 판정하는 것은 하나뿐입니다 — 화면에 실릴 그 숫자가 계단을
먹었는가.** 계단을 가로지르는 구간에서 생값(계열을 그대로 나눈 값)과
원천을 견주면 갈립니다.

| 결과 | 뜻 | 할 일 |
|---|---|---|
| 생값 ≈ 원천 | 네이버가 흘려 넣었다 | **비운다** |
| 생값 ≠ 원천 | 네이버가 보정했다 | 남긴다 |

문턱 20%p 는 임의로 고른 수가 아닙니다. 실물 20종목의 82개 칸을 재니
두 무리가 뚜렷하게 갈렸습니다.

```
흘려 넣은 칸 29개 — |차| 0.00 … 5.04 · 12.51 · 17.04
보정한 칸   53개 — |차| 30.07 · 30.47 · 35.80 … 198.98
```

17 과 30 사이가 비어 있고, 20 은 그 빈자리에 놓은 값입니다.

### 덤으로 갈린 것

- `base-price/chart` 의 `term` 은 `1m·3m·6m·1y·3y·5y` 만 받습니다
  (`9m·ytd·2y·10y·all·max` 는 HTTP 400). **길수록 성기게 솎여 옵니다** —
  3m 은 하루 간격 64점, 1y 는 주 간격 52점, 5y 는 달 간격 60점. 그래서
  수집기는 계열을 셋 받습니다. 하나만 받으면 계단을 놓치거나 구간을 못 덮습니다.
- `prices/daily` 의 `size` 상한은 **10** 입니다(100 부터 400).
- 기간수익률은 3년·5년도 **누적**입니다. ETF(네이버)가 연율이었던 것과
  다릅니다 — 이름이 같다고 뜻이 같지 않다는 것이 여기서도 맞았습니다.
- 내가 대조군으로 쓴 "원천 스스로의 앞뒤" 규칙(짧은 구간이 ±3% 안인데 긴
  구간이 ±50% 밖)도 **헛잡습니다.** 이 규칙이 잡은 두 펀드를 실물로 보니
  계단이 하나도 없었고, 최근 6개월이 잠잠하고 그 앞 6개월에 50% 오른
  정상적인 모양이었습니다. 그래서 감사에서 이 규칙은 **경고**입니다 —
  오류로 걸면 멀쩡한 자료가 커밋되지 못합니다.

---

## 이번 작업에서 새로 얻은 것 (2026-08-29)

ETF 목록에 없던 것만 적습니다. 근거는 `tools/discovery/fund_*_verify*.md`,
`fund_zero_weight.md` 에 있습니다.

### 가. `derivedNav / derivedAum == basePrice / 1000`

407종목 중 393종목이 상대오차 1% 안입니다. 곧 `derivedAum` 은 **설정원본**
(액면 1,000 기준)이고 `derivedNav` 는 현재 순자산이며, 둘의 비가 곧 기준가입니다.

이걸 모르고 "순자산이 설정액의 20배 밖이면 단위가 어긋난 것" 이라는 헐렁한
규칙을 걸었다가 인버스·2배 레버리지 7종목을 헛잡았습니다. 인버스 2배가 설정
이후 99% 빠지는 것은 이상한 일이 아닙니다 — 그 펀드의 0.0089배는 기준가 8.85
와 **정확히** 맞습니다.

항등식으로 바꾸니 헛잡음이 사라지고 규칙은 훨씬 세졌습니다. **필드의 뜻을
모르면 헐렁한 한도를 걸게 되고, 헐렁한 한도는 반드시 헛잡습니다.**

### 나. 반올림도 거짓말을 만든다

ETF 에서는 `Number(null) === 0` 이 입구였습니다. 이번에는 **반올림**이었습니다.

```
PETKIM PETROKIMYA   원천 0.000000045  →  toFixed(4)  →  0   →  화면 "0.00%"
```

원천은 0 을 준 적이 없습니다(표본 140종목에서 0 은 1건, null 은 0건).
파일 크기를 줄이려던 `toFixed(4)` 가 "담지 않았다" 는 거짓 진술을 만들었습니다.

**없는 것을 0 이라고 말하지 마라는 규칙은 null 만의 이야기가 아닙니다.**
0 이 되는 모든 경로를 보십시오 — null 강제변환, 반올림, 기본값, 빈 배열 합계.

화면도 같이 고쳐야 합니다. 저장된 값이 맞아도 `toFixed(2)` 가 다시 뭉갭니다.
지금은 0 이 아닌데 0.00% 로 반올림될 값을 `<0.01%` 로 찍습니다.

### 다. 분배가 잦은 펀드는 누적수익률이 원래 크다

하나클래스원특별자산투자신탁3 은 해마다 2월·8월에 기준가가 0.61~0.90배로
내려앉습니다(5년에 8번). 결산·분배입니다.

```
원천 5년          +1,517%
기준가 그냥 나눔     −1.07%
구간을 이어 붙임    +823%
```

셋 다 맞습니다. 네이버는 분배금을 재투자한 값을 주고, 기준가는 나눠 준 만큼
도로 내려갑니다. 한도 400% 를 걸었다가 이 정상값을 버릴 뻔했습니다 — ETF 에서
2배 레버리지의 1년 +843% 를 잡을 뻔한 자리와 같습니다.

**큰 수가 진짜인지는 임의의 한도가 아니라 그 펀드 자신의 기준가가 말해 줍니다.**

### 라. "없다" 는 판정에도 자리를 적어야 한다

`detail.totalFee` 가 비어 있는 것을 보고 "이 원천에는 총보수가 없다" 로
넘겼는데, **같은 응답의 한 단계 아래**(`returns.classes[]`)에 있었습니다.
화면과 README 와 각주에 "총보수는 만들 수 없다" 고 세 군데나 적어 놓은
뒤였습니다.

없다고 적을 때는 **어느 자리에서 없는지**까지 적으십시오. 그래야 다음 사람이
"그럼 다른 자리는?" 을 묻습니다.

### 마. 내 규칙은 이번에 다섯 번 틀렸다

세어 두는 편이 낫습니다. 한 번은 놓치는 쪽으로, 네 번은 헛잡는 쪽으로였습니다.

| 규칙 | 어떻게 틀렸나 | 어떻게 알았나 |
|---|---|---|
| 계단 배율을 하루당으로 환산 | 주말이 낀 3.147배를 **놓쳤다** | 계열을 눈으로 찍어 봄 |
| 순자산/설정액 20배 한도 | 인버스 7종목을 **헛잡음** | 필드의 뜻을 407종목으로 확인 |
| 수익률 400% 한도 | 분배형 1종목을 **헛잡음** | 계열의 톱니 모양을 봄 |
| 위험등급 1~6 숫자 | 3,195/3,196개를 **헛잡음** | 실제 값이 문자열 열거였음 |
| "비중 0 은 반올림이 샌 것" | 진짜 0 을 **헛잡음** | 제재로 평가가 0 이 된 러시아 종목 |

다섯 번 다 **실물을 보고서야** 알았습니다. 규칙이 무엇을 잡았는지 목록만 보고
고쳤다면 다섯 번 다 틀린 채로 갔을 것입니다.

특히 넷째는 감사가 **3,196개 중 3,195개**를 잡은 것이었습니다. 그 정도로
많이 잡히면 자료가 아니라 규칙이 틀린 것입니다 — 예외 없이.

---

## ETF 작업에서 얻은 것 — 그대로 가져가십시오

ETF 화면은 사용자가 "수익률이 엉터리야" 라고 지적하면서 1,348종목 전수
감사를 했고, **794종목에서 오류**가 나왔습니다. 같은 함정이 펀드에도 있습니다.

### 1. 전수 감사를 관문으로 걸 것

`scripts/audit_etf_data.mjs` 를 그대로 본떠 `audit_fund_data.mjs` 를
만드십시오. 바깥 자료에 붙지 않고 **데이터 안에서 서로 어긋나는 것만** 잡습니다.
안에서 앞뒤가 안 맞는 숫자는 바깥을 볼 것도 없이 틀린 것입니다.

일일 수집 워크플로의 커밋 **앞**에 두어, 오류가 있으면 커밋되지 않게 합니다.

### 2. `Number(null)` 은 `0` 이고 `Number.isFinite(0)` 은 `true`

이것 때문에 "비중 없음" 이 "비중 0%" 로 둔갑해 731종목의 화면이 거짓을
말했습니다. **null 여부를 먼저 보십시오.** 없는 것을 0 이라고 말하면
"안 담았다"·"겹치지 않는다" 는 거짓 진술이 됩니다.

펀드도 채권형·MMF 의 보유종목이 비므로 같은 자리가 옵니다.

### 3. 감사 규칙이 틀릴 수 있다 — 실물을 보고 물러설 것

처음 잡은 7,252건 중 대부분이 **제 규칙이 틀린** 오탐이었습니다.

| 잡은 것 | 실제 |
|---|---|
| 레버리지·선물형 비중 합 200% | 담보와 노출을 각각 적은 것 |
| 현금 −22% · 파생 −100% | 환매조건부·통화선도, 구조 그 자체 |
| 액티브 추적오차 25% | 지수를 그대로 안 따라가니 당연 |
| 1개월 내포분배율 연 56% | 연 1회 분배를 연율로 환산한 착각 |
| 2배 레버리지 1년 +843% | 기준가 +903% 와 맞는 실제 값 |

**마지막 것은 계산기 방어선에도 같은 착각이 들어가 있었습니다.** 연율로
걸었으면 정상값까지 버릴 뻔했습니다. 한도는 **누적**으로 거십시오.

### 4. 한 값이 깨지면 코호트 전체가 오염된다

총수익률 3,582% 짜리 한 종목이 국내 주식형 420종목의 유형 평균을 1년 기준
9.21%p 밀어 올렸고, 6개월은 **부호까지 뒤집었습니다**(+0.88% → −3.59%).
`audit_etf_data.mjs` 의 `유형평균-한종목이흔듦` 규칙을 가져가십시오.

### 5. 순위·유형평균은 상용 화면 관행을 따를 것

사용자가 모닝스타 기반 증권사 화면을 참조로 주었고, 그대로 맞췄습니다.

- 모집단은 **시장 전체가 아니라 동일 유형** 안에서
- 등수가 아니라 **백분율 순위 하나** (1 이 가장 좋음)
- **그 기간에 값이 있는 종목 수를 기간마다 따로** 적는다
- 유형 평균은 **산술평균 하나**. 중앙값을 같이 내는 곳은 없다

펀드는 `parentPeerGroupName`(해외주식형 등)이 **이미 유형 이름**이므로
ETF 처럼 우리가 분류를 지어낼 필요가 없습니다. 그걸 쓰십시오.

### 6. `continue-on-error` 가 죽음을 가린다

워크플로 단계가 **0초 만에 끝나고 success 로 찍히는** 일을 이 세션에서
두 번 겪었습니다. 스크립트에 `unhandledRejection` 처리를 넣어 지금까지
알아낸 것을 파일로 남기고 종료 코드 1 로 끝내십시오.

### 7. 이름이 같다고 뜻이 같지 않다

- 네이버가 "시장가" 라고 부른 것이 실은 **수정주가**였습니다.
- 상용 화면의 "PRICE(수정주가 기준)" 가 우리 `tr`(총수익률)과 소수점까지
  같았습니다. 우리 `price`(무보정 원가격)는 상용 화면에 나오지 않습니다.
- 같은 ETF 의 총보수가 0.07% 와 0.05%, 추적오차가 5.74 와 0.41 로 왔습니다.
  **정의가 다른 값**입니다.

계열을 쓰기 전에 그 계열이 무엇인지 먼저 확인하십시오.

---

## 만들 것

ETF 쪽 파일을 본뜨면 됩니다.

| ETF | 펀드 |
|---|---|
| `scripts/collect_etf_kr.mjs` | `scripts/collect_fund_kr.mjs` |
| `scripts/build_etf_data.mjs` | `scripts/build_fund_data.mjs` |
| `scripts/audit_etf_data.mjs` | `scripts/audit_fund_data.mjs` |
| `scripts/test_etf_page.mjs` | `scripts/test_fund_page.mjs` |
| `etf.html` → `etf-holdings-search.html` | `fund.html` → `fund-search.html` |
| `.github/workflows/etf-daily.yml` | `.github/workflows/fund-daily.yml` |

호출량은 목록 160 + 펀드당 2회(`left-panel`·`chart-price-panel`) ≈ **6,550회**
입니다. 동시 6개로 15~20분 걸립니다.

`scripts/etf_lib.mjs` 의 `getJson`·`mapLimit`·`writeDataFile`·`assertEnough` 를
그대로 쓰십시오. **네트워크 경로는 하나여야 합니다** — 검증이 수집기와 다른
경로로 부르면 검증의 뜻이 없습니다(야후 429 사고가 이것이었습니다).

### ETF 와 다른 점

- **시장가가 없습니다.** 펀드는 기준가뿐이라 시장가/총수익률 구분이 생기지
  않습니다. ETF 에서 크게 헤맨 자리가 펀드에는 없습니다.
- **벤치마크 대비**가 원천에 있습니다(`benchmarkReturn`). ETF 에는 없던 칸입니다.
- **클래스**(A/C/S 등)가 있습니다. `classes/returns` 로 옵니다.
- **위험지표**(샤프·젠센알파·베타)가 원천에 있습니다.
- **총보수가 없습니다.**

---

## 세션 이그레스

이 저장소의 세션은 KRX·네이버·야후에 직접 못 붙습니다(CONNECT 403).
러너는 그 정책 밖이므로, 조사·수집은 GitHub Actions 에서 돌리고 결과를
저장소에 커밋하는 방식으로 합니다. `.github/workflows/etf-probe.yml` 과
`etf-daily.yml` 이 그 본입니다 — `push` 로도 발동하게 되어 있습니다.
