...
모든 수치는 **조회 시점에 브라우저가 직접** 받아옵니다. 저장소에 시세를 쌓아두지 않습니다.

| 항목 | 소스 | 폴백 |
|------|------|------|
| 목록 시세(100종목) | Yahoo `v8/finance/spark` (25개 묶음) | Stooq CSV → 예시 데이터 |
| 상세 시세·일/주/월 차트·배당·분할 이벤트 | Yahoo `v8/finance/chart` (`events=div,split`) | 예시 데이터 |
| 지표·실적·컨센서스·일정·투자의견 | Yahoo `v10/finance/quoteSummary` (13개 모듈) | Yahoo `fundamentals-timeseries`(실적·밸류에이션만) → 예시 데이터 |
| 뉴스 | Yahoo `v1/finance/search` | Google News RSS → 딥링크 안내 |
| 원/달러 환율(시가총액 원화 환산) | Yahoo `chart/KRW=X` | 원화 환산 생략 |
...
- **실제 Yahoo/Stooq 응답으로는 확인하지 못했습니다**(이 저장소 작업 환경에서 외부 접속이 차단됨).
  사내 PC 브라우저에서 한 번 열어 배지가 `실시간`으로 뜨는지 확인해 주세요. 전부 `예시 데이터`로 뜨면
  ⑨ 섹션의 **요청 진단**에서 어느 경로가 막혔는지 볼 수 있습니다.
