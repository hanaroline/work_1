...
여러 증권사·매체의 아침 시황을 **한 화면으로 모아주는** 페이지입니다. 백엔드 없이 `market.html` 하나로 동작합니다.

**두 단계로 동작(점진적 향상)**
1. **모아보기 (API 키 불필요)** — **시황 수집**을 누르면 공개 소스를 CORS 프록시로 끌어와 그룹핑해 보여줍니다.
   - **네이버 금융 리서치 시황** — 증권사별 시황 리포트 제목/날짜(EUC-KR 자동 디코딩)
   - **Google 뉴스 RSS** — 시황 뉴스 헤드라인(검색어 편집 가능)
2. **AI 종합요약 (선택 · API 키 필요)** — **✦ AI 종합요약**을 누르면 모아온 제목들을 Claude가 중복 제거·공통 테마 우선으로 **한 장짜리 시황 브리핑**(핵심 포인트·시장 동향·오늘 체크포인트)으로 종합합니다. 키는 `briefing.html`과 공유됩니다.

- **하루 1회 캐싱** — 수집/요약 결과를 그날 날짜로 저장, 다시 열면 바로 표시
- **공유용 내보내기(⤓)** — 현재 화면(모아보기 + AI요약)을 **오프라인 독립형 HTML**로 저장 → 받는 사람은 키·클로드·인터넷 없이 더블클릭만으로 열람
...
### 다른 사람과 공유하기 — 공유용 내보내기 (⤓)

받는 사람이 **Claude 계정·API 키 없이** 그냥 파일을 더블클릭만 해서 열람하게 하려면:

1. `briefing.html`에서 그날 **번역·요약을 생성**합니다.
2. 상단 **⤓ 공유용 내보내기**를 누르면 `bloomberg-briefing-YYYY-MM-DD.html` 파일이 다운로드됩니다.
3. 이 파일은 **오늘 내용이 통째로 박힌 완전 독립형 뷰어**입니다 — 인터넷·API·키·클로드 없이 오프라인에서 열립니다.
4. 이 파일을 메신저·메일·사내 드라이브로 공유하면, 받는 사람은 더블클릭만 하면 됩니다(한/영 토글·인쇄 그대로 지원).

> 요약하면: **생성은 보내는 사람이 briefing.html로(키 필요), 열람은 받는 사람이 내보낸 파일로(아무것도 불필요).**
