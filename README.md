...
**기본은 총수익률(Total Return)** 입니다 — 분배금을 재투자한 기준으로, 펀드 성과를
재는 업계 표준입니다.

| 계열 | 뜻 | 범위 |
|---|---|---|
| `ret.tr` | **총수익률** · 분배금 재투자 | 국내·해외 공통 · **기본 기준** |
| `ret.price` | 시장가 등락률 · 분배금 제외 | 국내·해외 공통 |
| `ret.nav` | NAV(기준가) 등락률 · 분배금 제외 | 국내만 |

`tr` 은 **국내·해외가 같은 계산기**(`etf_lib.mjs` 의 `computeReturns`)로 야후 일별
수정종가에서 산출합니다. 그래서 두 시장을 한 줄로 세워 순위를 매겨도 됩니다.
3년·5년은 연율입니다.

`price` 는 거래소 종가의 단순 등락률입니다. HTS에서 보는 숫자와 같지만 **분배금이
빠져 있어** 커버드콜·고배당 상품은 실제 성과보다 낮게 보입니다. 화면은 기준을
바꿀 때마다 이 사실을 안내문으로 띄우고, 분배율 5% 이상 상품의 상세에는 경고를
붙입니다.

`nav` 는 네이버가 산출한 기준가 수익률입니다. **여기에도 분배금은 반영되지
않습니다** — 시장가와의 차이는 괴리율뿐입니다(분배율 4.6% 표본에서 차이 중앙값
0.02%p). 국내 상세에만 참고로 싣습니다.

#### 왜 국내 총수익률을 따로 받는가

네이버는 총수익률을 주지 않습니다. 두 계열 모두 분배금이 빠져 있어, 분배율이
20%를 넘는 커버드콜의 성과가 통째로 사라집니다.

```
PLUS 고배당주위클리커버드콜   분배율 23.3%   시장가 1년 −24.08%   분배금 포함 시 약 −0.7%
```

국내에만 분배율 5% 이상이 72종목입니다. 그래서 야후 수정종가(`{종목코드}.KS`)로
국내분 총수익률을 따로 받습니다. 야후에 없는 종목은 `tr` 이 비고, 수집 로그와
`withTr` / `trMissing` 에 남습니다.
...
분배율을 지렛대로 삼아 `ret.tr` 이 정말 총수익률인지 판정합니다 — 분배금이
반영됐다면 `tr − price` 가 분배율만큼 나와야 합니다. 시장마다 뜻이 다르거나
분배금이 반영되지 않으면 실패로 끝나고, 수집 워크플로도 여기서 멈춥니다.
화면의 기본 기준이 거짓이 되는 것을 막는 자리입니다.
