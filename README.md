...
`ret` 은 두 계열을 담지만 **두 번째 계열의 뜻이 시장마다 다릅니다.** 이는 추정이
아니라 `scripts/check_etf_return_basis.mjs` 로 검산한 사실입니다.

| | `ret.price` | `ret.nav` |
|---|---|---|
| **국내** | 시장가(종가) 등락률 · 분배금 제외 | NAV(기준가) 등락률 · **분배금 제외** |
| **해외** | 시장가(종가) 등락률 · 분배금 제외 | 수정종가 기준 **총수익률 · 분배금 재투자** |

검산 결과(1년 기준, 분배율 3% 이상 종목):

```
시장   표본  분배율(중앙)  두 계열 차이  차이/분배율  판정
KR      130         4.63         0.02        0.01  분배금 미반영 (차이는 괴리율뿐)
US       31         4.47         4.23        0.95  분배금 재투자 (총수익률 TR)
```

즉 **국내는 두 계열의 차이가 괴리율뿐**이고, **해외는 차이가 곧 분배금**입니다.
같은 열에 다른 개념이 들어 있으므로 이 기준으로 국내와 해외를 맞대어 비교하면
안 됩니다. 화면은 기준을 바꿀 때마다 이 사실을 안내문으로 띄우고, 상세의 열
이름도 시장별로 다르게 적습니다.

**시장가 기준은 두 시장이 같은 방식**이라 한 줄로 세울 수 있습니다. 다만 분배금이
빠져 있어 커버드콜·고배당 상품은 실제 성과보다 낮게 보입니다(국내 분배율 5% 이상
72종목). 예: `PLUS 고배당주위클리커버드콜`은 1년 시장가 수익률이 −24.08%지만
분배율 23.3%를 더하면 대략 보합입니다. 상세를 열면 분배율과 경고가 함께 나옵니다.

**업계 표준은 분배금을 재투자한 총수익률(Total Return)** 입니다. 국내 총수익률을
확보하려면 야후 수정종가(`.KS`/`.KQ`)로 국내분도 계산해야 하며, 그때 비로소 두
계열이 한 뜻이 됩니다. 현재는 미구현입니다.
