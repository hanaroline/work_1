...
스냅샷을 찾는 순서는 `SNAPSHOT_BASES` 에 있습니다 — ① 같은 폴더의 상대 경로
`data/us100/`(저장소를 그대로 열거나 서비스할 때), ② `raw.githubusercontent.com` 의
`main` 브랜치(HTML 파일만 따로 받아 열었을 때). 사내 주소로 바꾸려면 화면 ⑨ 섹션의
**데이터 경로 설정**에 넣거나 이 배열을 고칩니다.
