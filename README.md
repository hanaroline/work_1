...
상품을 선택하면 각 항목에 투자설명서 내용이 자동 반영됩니다. 조회 경로는 4단계이고,
위에서부터 가능한 것을 쓰면 됩니다.
...
투자설명서 · 간이투자설명서 · 핵심(요약)설명서 PDF를 올리면 텍스트를 읽어 항목을
자동 추출합니다. `vendor/pdf.min.js` 를 내장하고 있어 외부 네트워크가 없어도 동작합니다.

추출 결과는 **항목 · 추출값 · 근거 문구**를 한 표로 보여주고, 담당자가 확인 후
「반영」을 눌러야 스크립트에 들어갑니다. 근거 없이 값을 확정하면 부정확한 설명으로
부당권유행위 감점 사유가 되기 때문입니다. 「전체 반영」으로 한 번에 넣을 수도 있습니다.
...
상품 기본정보와 담당자 입력값을 하나의 JSON으로 주고받습니다. 같은 상품을 여러 지점에서
쓸 때 한 번만 채워 공유하면 됩니다.
...
js/sales-script-prospectus.js  # 투자설명서 조회 (PDF 추출규칙 · 사내 API 어댑터)
