...
market-monitor.html   # 마켓 모니터 — 시장 지표 추이 (빌드 결과물, 아래 참고)
portfolio-check.html  # 포트폴리오 점검 (빌드 결과물, 아래 참고)
docs/index.html       # 브리핑 아카이브 (빌드 결과물, 아래 참고)
docs/briefings/       # 발행한 브리핑 보관본 + index.json
docs/briefing-playbook.md # 시황 브리핑 작성 지침
data/market/          # 시세 수집 결과 (latest.json · 날짜별 · history.json 누적본)
scripts/masviz.py     # 브랜드 공통 조각 — 숫자 표기·차트·표 (세 화면이 공유)
...
`market-monitor.html` — 브리핑이 **어제 무슨 일이 있었나**를 답한다면, 이 화면은
**요즘 흐름이 어떤가**를 답합니다. 수집기가 하루 3~4번 188개 소스를 받는데 그 값이
그날 브리핑 한 장에만 쓰이고 버려지던 것을 추이로 세운 것입니다.

| 절 | 내용 |
|---|---|
| **시장 온도** | 코스피·코스닥·원/달러·VIX·미 국채 10년·10−2 스프레드·예탁금·신용잔고·거래대금 타일 |
| **국내 지수 추이** | 코스피·코스닥 리베이스 선 + 거래대금(축을 따로 세움) |
| **등락 종목 수** | 상승·하락을 0 위아래로. 지수가 올라도 오른 종목이 적으면 상승의 폭이 좁다 |
| **투자자별 순매수** | 외국인·기관·개인 29거래일 + 5/20일 누적 |
| **예탁금 · 신용잔고** | 수준이 3배 이상 달라 이중축 대신 차트 둘로 나눔 |
| **업종 강약** | 상위·하위 8개 업종의 일별 등락률 히트맵 |
| **미 국채 커브** | 현재·1주 전·1개월 전 곡선 + 주간 변화(bp) |
| **국내 금리 추이** | 국고 3·10년, 회사채 3년, CD 91일 |
| **환율 · 원자재** | 기간 수익률 표 |
| **데이터 상태** | 받지 못한 소스와 사유를 그대로 노출 |

두 곳에서 읽습니다.

- `data/market/latest.json` — 그날 스냅숏. 안에 이미 여러 날짜가 들어 있습니다:
  `index_daily`(20거래일), `investors_kospi`(29일), `money_flow.series`(20일),
  그리고 모든 지수·종목·업종의 `perf`(w1/m1/m3/m6/y1/ytd)
- `data/market/history.json` — 스냅숏에 시계열이 없는 항목(등락 종목 수·업종
  등락률·금리 커브·거래대금)의 날짜별 누적본. 하루 한 행씩 늘고 120거래일까지 남습니다

```bash
python3 scripts/build_market_monitor.py    # → market-monitor.html (더블클릭으로 열림)
```

시세 수집 워크플로우(`market-data.yml`)가 수집 직후 이 파일을 다시 만들어 커밋합니다.
**기간이 짧은 절은 누적본이 아직 쌓이는 중이라는 뜻입니다** — 각 절 제목에 누적
거래일 수를 적어 둡니다.
...
`portfolio-check.html` — 보유 종목·ETF 를 입력하면 한 장으로 정리합니다.

| 절 | 내용 |
|---|---|
| **보유 내역** | 종목 자동완성(수집 대상 109종) · 수량 · 평균단가. 목록 붙여넣기 지원 |
| **요약** | 평가금액, 평가손익, 수익률, 종목 수, 최대 비중, 집중도 HHI |
| **비중 · 손익** | 종목별 비중 막대 + 평가금액·손익·수익률 표 |
| **배분** | 지역 · 통화 · 유형 3분할 |
| **기간 성과** | 보유 가중 1주~1년 vs 코스피·코스닥·S&P500·나스닥 |
| **집중도 진단** | 단일 20% · 상위3 50% · 단일 지역 70% · 외화 40% 기준선 대조 |
| **ELS 낙인 여력** | 기초자산·기준일·낙인·배리어를 넣으면 현재 수준, 기간 중 최저, 낙인 터치 여부 |

```bash
python3 scripts/build_portfolio_report.py   # → portfolio-check.html
```

> **입력한 보유 내역은 브라우저를 벗어나지 않습니다.** 서버로 전송하지 않고
> `localStorage` 에만 저장합니다. 고객 자산 정보를 다루므로 이 성질을 깨는 변경은
> 하지 마십시오. 공용 PC 에서는 「전체 지우기」로 지웁니다.

평가금액은 **마지막 수집 종가**와 표시된 환율로 계산한 값이라 실제 계좌 잔고와
다릅니다. 세금·수수료·배당은 반영하지 않습니다. 기간 성과는 「지금 비중 × 각 종목의
과거 기간수익률」이므로 그 기간 내내 이 비중을 들고 있었다는 가정이며 실제 수익률이
아닙니다. 집중도 기준선은 널리 쓰이는 참고값이고 감독 규정이나 사내 기준이 아닙니다.

원화 환산 환율은 `usdkrw_naver`(매매기준율)를 기준으로 만들고, 크로스는 재정환율로
계산합니다. 환율을 만들 수 없는 통화의 보유는 합계에서 빼고 화면에 그 사실을
적습니다(수집기에 `usdhkd`·`usddkk` 를 추가했으므로 다음 수집부터 홍콩·덴마크
상장분도 환산됩니다).
...
`docs/index.html` — 발행한 모닝 · 장마감 · 해외 증시 · 주간 브리핑 보관본을 판 종류로
걸러 보고 **제목·소제목·본문으로 검색**합니다. 맞은 자리를 앞뒤 문맥과 함께 보여주고,
행을 누르면 본문이 그 자리에서 열립니다.

```bash
python3 scripts/build_briefing_archive.py   # → docs/index.html + docs/briefings/search-index.json
python3 scripts/build_weekly_digest.py      # → docs/briefings/<날짜>-weekly.html (금요일 판)
```

보관본은 `<html>`·`<body>` 없는 **아티팩트 본문 조각**입니다. 그냥 링크하면 서버가
charset 을 주지 않아 한글이 깨지므로, 아카이브는 본문을 `fetch` 로 받아 UTF-8 로 직접
디코딩해 iframe 에 넣습니다. 「새 창에서 열기」도 디코딩한 blob 을 가리킵니다.
**목록과 검색은 색인을 셸 안에 박아 넣어 파일로 열어도(file://) 동작하고, 본문 보기만
HTTP 가 필요합니다.**

주간 다이제스트는 `latest.json` 하나로 한 주를 정리해 다른 브리핑과 같은 규칙의 본문
조각으로 쓰고 `index.json` 에 등록하므로, 아카이브의 목록·검색·본문 보기가 그대로
걸립니다. `weekly-digest.yml` 이 금요일 16:30 KST 에 만들고, 브리핑이 새로 보관되면
아카이브 색인을 다시 만듭니다.
...
브랜드 색은 고정값(`#F58220` / `#043B72`)이라 팔레트 검증기의 두 항목을 통과시킬 수
없습니다 — `#043B72` 는 명도 밴드보다 어둡고 `#F58220` 의 표면 대비는 2.53:1 입니다.
색을 「비슷한 값」으로 바꾸지 않고, 검증기가 요구하는 완화 조건을 지킵니다.

- 한 차트의 **계열은 4개 이하** (4개까지는 색약·정상시야 분리도가 모두 통과)
- **등락은 상승 `#C62828` / 하락 `#043B72`** 다이버징 페어 (둘 다 대비 3:1 통과, 국내 표기 관행과도 맞음)
- 대비가 낮은 색에 크기 판단을 맡기지 않습니다 — **모든 차트에 수치 라벨을 직접 달고 같은 값을 표로도** 싣습니다
- **축은 언제나 하나**입니다. 단위가 다른 두 계열은 리베이스하거나 차트를 나눕니다(이중축 금지)
- **상태값(안전/위험)은 등락 색과 섞지 않고** 예약 팔레트(`#2E8540` / `#C62828` / `#D4A017`)를 쓰고 문구를 함께 답니다

숫자 표기·차트·표·툴팁은 `scripts/masviz.py` 에 모여 있고 세 화면이 공유합니다.
포트폴리오 점검만 입력이 런타임에 들어오므로 같은 규칙을 JS 로 한 번 더 구현합니다.
