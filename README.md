...
`ret` 의 기준이 두 시장에서 다릅니다. **국내**는 네이버가 산출한 시장가/NAV 수익률을
그대로 싣고, **해외**는 일별 종가(시장가)와 수정종가(NAV 대용, 분배금 반영)로 이
페이지가 계산합니다. 3년·5년은 연율입니다. 산출 방식이 달라 두 시장의 수치를
소수점까지 맞대어 비교하면 안 됩니다.
