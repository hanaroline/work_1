...
var MASPSRC = (function () {
  ...
  /* 실제 응답(/search/getTotalSearch.jsp) 확인 결과
       DOCID 펀드표준코드(KR...)  FDN 펀드명       ADMICN 운용사명(코드 아님)
       ADMIC_CD 운용사코드        PD_CLSS 위험등급  PD_TYP_CD 유형코드
       Date 설정일               M1_BNFR/M6_BNFR/M12_BNFR 기간수익률
       ESTD_AF_BNFR 설정후수익률  ONLINE_PRVT_YN 온라인판매
       AREA_DVN/DMS_FO_DV_CD 지역구분
     모바일 응답과 필드가 달라(KR_FD_CD/NASST_SUM/HAN_CLAS_NM 등) 양쪽 모두
     후보로 두고, 없는 값은 채우지 않는다. */
  function masFundRow(fd) {
    var nm = String(fd.FDN || '').split('\r')[0].trim();
    if (!nm) return null;
    var grade = parseInt(String(fd.PD_CLSS || '').replace(/[^0-9]/g, ''), 10);
    var nasst = n(pick(fd, ['NASST_SUM', 'NASST_TOTAMT']));
    var online = String(pick(fd, ['ONLINE_PRVT_YN']) || '').trim() === '1';

    /* ADMICN 은 응답에 따라 이름이거나 코드다. 숫자만이면 코드로 보고 비워 둔다. */
    var adm = String(fd.ADMICN || '').trim();
    var provider = /^[0-9\s]*$/.test(adm) ? '' : adm;

    var region = null;
    var dms = String(pick(fd, ['DMS_FO_DV_CD', 'AREA_DVN']) || '').trim();
    if (/^1$|국내|domestic/i.test(dms)) region = 'kr';
    else if (/^2$|해외|oversea|global/i.test(dms)) region = 'gl';

    return {
      cat: 'fund', live: true, source: '미래에셋증권',
      sourceUrl: 'https://securities.miraeasset.com/hks/hks4116/r01.do',
      code: String(pick(fd, ['KR_FD_CD', 'DOCID']) || '').trim(),
      name: [nm, nm],
      provider: [provider, provider],
      className: String(pick(fd, ['HAN_CLAS_NM']) || '').trim(),
      risk: (grade >= 1 && grade <= 6) ? grade : null,
      ret1m: n(fd.M1_BNFR), ret3m: n(fd.M3_BNFR),
      ret6m: n(fd.M6_BNFR), ret1y: n(fd.M12_BNFR),
      ret3y: null,
      sinceLaunch: n(fd.ESTD_AF_BNFR),
      /* NASST_SUM 은 원 단위 -> 화면 단위(억원). 응답에 없으면 비워 둔다. */
