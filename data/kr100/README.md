...
| 이 폴더에 남아 있는 것 | 역할 |
|---|---|
| `latest.json` · `quotes.json` · `list-change.json` | `{"movedTo": ...}` **안내 파일**. 예전 경로를 보고 있는 화면이 새 위치를 찾아갑니다 |
...
| `WEEKLY_REFRESH` | 파일을 고쳐 push 하면 **주간 목록 자동 갱신**이 돕니다 |
...
| `list-change.json` | 주간 목록 자동 갱신에서 무엇이 들고 났는지 (화면 ⑩ 섹션이 한 줄로 보여 줍니다) |
...
대상 목록은 **금요일 17:00 KST 에 스스로 갱신됩니다** — 시총 90위 안에 들면 들어오고
110위 밖으로 밀리면 나가며, 한 번에 다섯 종목까지 바뀝니다. 새로 들어온 종목의 한글
사명·검색 키워드·기업 개요는 수집한 값으로 지어 함께 넣고, 화면이 "사람이 쓴 요약이
아니다" 라고 밝힙니다. 자세한 것은 저장소 README 의 *대상 종목은 매주 저절로 바뀝니다* 를
보세요.
