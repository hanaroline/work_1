...
# 없다. 브라우저를 몰아 쓰는 것들 — 화면 연기 시험, 수집기, PDF·덱 만들기 —
# 이 playwright 를 부르므로, 그것이 없으면 세션이 시작하자마자 "모듈을 찾을
# 수 없다" 로 막힌다. 여기서 미리 깔아 둔다.
#
# 가지마다 쓰는 대본은 다르지만 없는 것은 똑같다. 그래서 이 파일은 어느
# 가지에서도 같은 내용이다 — 갈라 두면 한쪽만 고쳐지고 다른 쪽은 낡는다.
#
