...
function render() {
  ...
  L.push('| 구 계좌 → 기존 신 계좌 | 가능 | **6년차 → 1년차** | **옮기지 말 것** - 한도가 몇 배 줄어듭니다 |');
  L.push('| 구 계좌 → 신규 개설 계좌 (전액) | 가능 | 승계 가능(미확인) | 가입일자 승계 여부를 반드시 확인 |');
  L.push('| 신 계좌 → 기존 구 계좌 | **불가** | - | Q23② 에 걸립니다 |');
  L.push('| 신 계좌 → 신 계좌 | 가능 | 1년차 그대로 | 무방 (수수료·상품만 보면 됩니다) |');
  L.push('');
  L.push('> 세 번째 줄의 가입일자 승계는 원본 Q&A 를 확인하지 못했습니다. 소득세법 시행령 §40의4② 로');
  L.push('> 알려진 내용(새로 설정된 계좌에 전액 이체 시 이체 전 계좌 기준)이지만, **확인 전까지는');
  L.push('> 상담에 단정해 쓰지 마세요.**');
  L.push('');
